 ...
 * graveyard_source[0][2] & 0x00FFFE00 will be its unique id (3, unless more cards than those first two have been put in a graveyard that game), and
...
enum {
  DECK_MAX_PLAYERS = 2,
  DECK_MAX_CARD_SLOTS = 150,
  DECK_MAX_ZONE_CARDS = 500
};

static int deck_player_is_valid(int player)
{
  return player >= HUMAN && player <= AI;
}

static int deck_card_slot_is_valid(int card)
{
  return card >= 0 && card < DECK_MAX_CARD_SLOTS;
}

static int deck_card_ref_is_valid(int player, int card)
{
  return deck_player_is_valid(player) && deck_card_slot_is_valid(card);
}

static int deck_zone_position_is_valid(int position)
{
  return position >= 0 && position < DECK_MAX_ZONE_CARDS;
}

static int deck_zone_has_card(const int* zone, int position)
{
  return zone && deck_zone_position_is_valid(position) && zone[position] != -1;
}

static int find_free_zone_slot_from(const int* zone, int start)
{
  if (!zone)
    return -1;
  ...
  int i;
  for (i = start; i < DECK_MAX_ZONE_CARDS; ++i)
    if (zone[i] == -1)
      return i;
...
static int find_free_zone_slot(const int* zone)
{
  return find_free_zone_slot_from(zone, 0);
}

static int calc_initial_attack_rating_common(int player, int iid, int state)
{
  /* This mirrors Shandalar's AI creature valuation.  Keep the arithmetic close to the exe, since small changes here can shift combat and casting choices. */
  if (!deck_player_is_valid(player) || iid < 0)
    return 0;
  ...
  if ((state & STATE_TAPPED) && current_turn == player)
    --rating;
...
int calc_initial_attack_rating_by_iid(int player, int iid)
{
  ...
  return calc_initial_attack_rating_common(player, iid, 0);
}

static int calc_initial_attack_rating(int player, int card)
{
  // 0x479880, called only from create_card_instance().
  if (!deck_card_ref_is_valid(player, card))
    return 0;

  card_instance_t* instance = get_card_instance(player, card);
  if (instance->internal_card_id < 0 || (instance->state & STATE_OUBLIETTED))
    return 0;

  return calc_initial_attack_rating_common(player, instance->internal_card_id, instance->state);
}
...
void create_card_instance(int player, unsigned int iid, int card)
{
  ...
  ASSERT(deck_card_ref_is_valid(player, card));
...
{
  if (!deck_zone_has_card(deck, pos))
    return;

  int i;
  for (i = pos; i < DECK_MAX_ZONE_CARDS - 1 && deck[i] != -1; ++i)
    deck[i] = deck[i + 1];

  deck[DECK_MAX_ZONE_CARDS - 1] = -1;
}

// Remove the top card of player's library, without putting it anywhere else.
int obliterate_top_card_of_deck(int player)
{
  if (!deck_player_is_valid(player))
    return -1;
...
{
  if (!deck_player_is_valid(player) || number <= 0)
    return;
  ...
  number = MIN(number, DECK_MAX_ZONE_CARDS);

  int i;
  int end = DECK_MAX_ZONE_CARDS - number;
  for (i = 0; i < end && deck[i] != -1; ++i)
    deck[i] = deck[i + number];

  for (; i < DECK_MAX_ZONE_CARDS; ++i)
    deck[i] = -1;
}
...
{
  // 0x477f30.
  if (!deck_player_is_valid(player) || !deck_zone_position_is_valid(position))
    return;
...
int can_draw_cards_as_cost(int player, int amount)
{
  ...
  if (amount > DECK_MAX_ZONE_CARDS || deck_ptr[player][amount - 1] == -1)
    return 0;
...
{
  if (!deck_player_is_valid(player) || maximum <= 0)
    return 0;

  int how_many;
  if (player == AI || ai_is_speculating == 1)
    how_many = MAX(0, MIN(count_deck(player) - 20, maximum));
  else
...
{
  // action = 1 -> Put on top; 2 -> Put on bottom; 3 -> Shuffle into deck.
  if (!deck_player_is_valid(player) || !deck_zone_position_is_valid(selected))
    return 0;

  const int *grave = get_grave(player);
  if (!grave || grave[selected] == -1)
    return 0;

  int iid = grave[selected];
  ...
  if (action == 2 || action == 3)
    {
      int *deck = deck_ptr[player];
      int deck_pos = count_deck(player);
      if (!deck || deck_pos < 0 || deck_pos >= DECK_MAX_ZONE_CARDS)
        return 0;
  ...
  if (is_hydra_head(player, card))
  ...
  else
    {
      if (in_hand(player, card))
        --hand_count[player];
      real_put_on_top_of_deck(player, card);
      if (!(get_card_instance(player, card)->token_status & (STATUS_TOKEN | STATUS_OBLITERATED)))
        {
...
{
  if (!deck_player_is_valid(player) || position < 0)
    return -1;

  int* deck = deck_ptr[player];
  int count = count_deck(player) - 1;
  if (!deck || count < position)
    return -1;
...
int reshuffle_grave_into_deck(int player, int avoid_shuffle)
{
  ...
  int amount = 0;
  while (count > -1 && d_count < DECK_MAX_ZONE_CARDS)
    {
...
void reshuffle_all_into_deck(int player, int mode)
{
  ...
  // 0 -> normal mode; 1 -> "Restart the game" mode (Karn Liberated); 2 -> put everything back to deck but don't reshuffle.
  int edh_general_iid = -1;
  int commander_found = 0;
  int count = active_cards_count[player] - 1;
  int planes[DECK_MAX_CARD_SLOTS];
  int pc = 0;
  while (count > -1)
    {
      if (in_play(player, count))
        {
          if (is_what(player, count, TYPE_PERMANENT))
          ...
          else if (mode & 1)
            {
              ...
              if (is_plane_card(get_id(player, count)) && pc < DECK_MAX_CARD_SLOTS)
                {
                  remove_counters(player, count, COUNTER_ENERGY, count_counters(player, count, COUNTER_ENERGY));
                  planes[pc++] = count;
                }
      ...
      if (put_into_deck)
        {
          ...
          if (pos < DECK_MAX_ZONE_CARDS)
            deck[pos] = grave[count];
          remove_card_from_grave(player, count);
        }
      --count;
    }
  if (!commander_found && edh_general_iid != -1)
    {
      count = 0;
      while (count < DECK_MAX_ZONE_CARDS && deck_ptr[player][count] != -1)
        {
  ...
  if (kill_mode == KILL_REMOVE && !(flags & FROM_EXE) && !is_what(player, card, TYPE_EFFECT))
    {
      ...
      if (!(cards_data[instance->original_internal_card_id].type & TYPE_CREATURE))
        play_sound_effect(WAV_DESTROY);  // This is the rfg sound effect, despite its name.
    }
...
int destroy_attached_auras_and_obliterate_card(int player, int card)
{
  ...
  int p, c, count = 0;
  for (p = 0; p < 2; ++p)
    for (c = 0; c < MIN(active_cards_count[p], DECK_MAX_CARD_SLOTS); ++c)
      if (!(p == player && c == card) && in_play(p, c))
        {
...
static int raw_put_iid_on_top_of_graveyard_impl(int player, int iid, int position_hint, int32_t source_val)
{
  ...
  int start = 0;
  if (position_hint > 0 && position_hint < DECK_MAX_ZONE_CARDS && grave[position_hint - 1] != -1)
    start = position_hint;

  int pos = find_free_zone_slot_from(grave, start);
  if (pos == -1)
    return -1;

  grave[pos] = iid;
  graveyard_source[player][pos] = source_val;
  return pos;
}
...
{
  ASSERT(deck_player_is_valid(player));
  ASSERT(deck_zone_has_card(rfg_ptr[player], position_in_exile));
...
/* Replaces the card in player's graveyard at position position with the card back image, and returns the internal_card_id that was there.  It's the caller's
...
{
  ASSERT(deck_player_is_valid(player));
  ASSERT(deck_zone_has_card(graveyard_ptr_mutable[player], position));

  int previous = graveyard_ptr_mutable[player][position];
  ASSERT(previous != iid_draw_a_card);
...
{
  ASSERT(deck_player_is_valid(player));
  ASSERT(deck_zone_position_is_valid(position));
...
{
  if (!deck_player_is_valid(player) || deck_ptr[player][0] == -1)
    return 0;
...
static int rfg_card_from_deck_impl(int player, int* deck, int pos)
{
  ...
  int rfg_pos = find_free_zone_slot(rfg_ptr[player]);
  if (rfg_pos == -1)
    return -1;

  int iid = deck[pos];
  rfg_ptr[player][rfg_pos] = iid;
...
{
  if (!deck_player_is_valid(player) || !deck_zone_has_card(graveyard_ptr_mutable[player], grave_id))
    return -1;

  // Rakshasa Vizier effect.
  int c;
  int active_count = MIN(active_cards_count[player], DECK_MAX_CARD_SLOTS);
  ...
  int exiled = rfg_card_from_deck_impl(player, graveyard_ptr_mutable[player], grave_id);
  if (exiled != -1)
...
{
  if (!deck_player_is_valid(player) || !deck)
    return 0;
  ...
  while (deck[0] != -1)
    {
      ...
      rfg[rfg_pos] = deck[0];
      obliterate_card_from_deck_impl(deck, 0);
      ++num_exiled;
    }
...
int rfg_whole_graveyard(int player)
{
  ...
  int result = 0;
  while (graveyard_ptr_mutable[player][0] != -1)
    {
      int exiled = rfg_card_from_deck_impl(player, graveyard_ptr_mutable[player], 0);
      if (exiled == -1)
        break;
      obliterate_card_from_deck_impl(graveyard_source[player], 0);
      ++result;
    }
...
int add_card_to_rfg(int player, int iid)
{
  ...
  int pos = find_free_zone_slot(rfg_ptr[player]);
  if (pos == -1)
    return -1;

  rfg_ptr[player][pos] = iid;
  return pos;
}
...
int find_iid_in_rfg(int player, int iid)
{
  ...
  int* rfg = rfg_ptr[player];
  int i;
  for (i = count_rfg(player) - 1; i >= 0; --i)
    if (rfg[i] == iid)
      return i;
  return -1;
}
...
{
  if (!deck_player_is_valid(player) || !deck_zone_position_is_valid(count))
    return;
...
{
  if (!deck_card_ref_is_valid(player, card) || !is_what(player, card, TYPE_PERMANENT) || !in_play(player, card))
    return -1;
...
{
  if (!deck_player_is_valid(player) || !deck_player_is_valid(t_player) || !deck_zone_has_card(deck_ptr[t_player], deck_position))
    return -1;
...
static int effect_replace_self_mill(int player, int card, event_t event)
{
  ...
  if ((xtrigger_condition() == XTRIGGER_REPLACE_MILL || xtrigger_condition() == XTRIGGER_REPLACE_CARD_TO_GY_FROM_ANYWHERE_BUT_LIBRARY)
      ...
      && trigger_cause_controller == player)
    {
      ...
      if (iid < 0 || cards_data[iid].id != csvid)
        return 0;
...
void special_mill(int player, int card, int source_csvid, int orig_t_player, int amount)
{
  ...
  amount = MIN(amount, DECK_MAX_ZONE_CARDS);
      ...
      if (orig_t_player == k || orig_t_player == ANYBODY)
        {
          ...
          for (i = 0; i < amount && deck[0] != -1; ++i)
            {
              ...
              if (!deck_zone_has_card(deck, deck_pos))
                break;
              ...
              /* Part of the milling effect, not a separate trigger.  Unlike Helm of Obedience, it happens even if the library-to-graveyard effect is replaced,
               * so long as the card is moved to a public zone. */
          ...
          if (source_csvid == CARD_ID_STITCHER_GERALF)
            {
              card_instance_t* instance = get_card_instance(player, card);
          ...
          if (source_csvid == CARD_ID_HERETICS_PUNISHMENT)
            {
              card_instance_t* instance = get_card_instance(player, card);
...
int whenever_type_is_put_into_graveyard_from_anywhere(int player, int card, event_t event, int t_player, int type, test_definition_t *test)
{
  // From library.
  enable_xtrigger_flags |= ENABLE_XTRIGGER_MILLED;
  if (xtrigger_condition() == XTRIGGER_MILLED && affect_me(player, card) && reason_for_trigger_controller == player
      && (trigger_cause_controller == t_player || t_player == ANYBODY))
    {
      ...
      for (i = 0; i < num_cards_milled; ++i)
        if ((type && is_what(-1, cards_milled[i].internal_card_id, type))
      ...
      if (test == NULL)
        {
          ...
          if (when_a_card_is_put_into_graveyard_from_anywhere_but_library_do_something(player, card, event, t_player, RESOLVE_TRIGGER_MANDATORY, &test2))
            return 1;
        }
      else if (when_a_card_is_put_into_graveyard_from_anywhere_but_library_do_something(player, card, event, t_player, RESOLVE_TRIGGER_MANDATORY, test))
        return 1;
    }
...
{
  // mode >= 0: forward to reanimate_permanent; -1: return to hand, return new card; -2: leave in graveyard, return position.
  if (!deck_player_is_valid(t_player) || id < 0)
    return -1;
  ...
  while (count > -1)
    {
      if (cards_data[grave[count]].id == id)
        {
          if (mode >= 0)
            return reanimate_permanent(player, card, t_player, count, mode);
          else if (mode == REANIMATEXTRA_RETURN_TO_HAND2)
            {
              int card_added = add_card_to_hand(t_player, grave[count]);
...
int new_select_target_from_grave(int player, int card, int t_player, select_grave_t mode, int ai_mode, test_definition_t *this_test, int ret_location)
{
  ...
  if (selected != -1)
    {
      const int *grave = get_grave(t_player);
      if (deck_zone_has_card(grave, selected))
        {
...
int select_target_from_grave_source(int player, int card, int t_player, select_grave_t mode, int ai_mode, test_definition_t *this_test, int ret_location)
{
  ...
  if (selected == -1)
    ...
    {
      if (!deck_zone_has_card(get_grave(t_player), selected))
        {
          if (!(mode & SFG_NO_SPELL_FIZZLED))
            cancel = 1;
          return -1;
...
int validate_target_from_grave(int player, int card, int t_player, int targ_location)
{
  ...
  if (deck_zone_has_card(grave, selected) && grave[selected] == instance->targets[targ_location].card)
    return selected;
  return -1;
}
...
{
  if (!deck_player_is_valid(player) || !deck_zone_has_card(get_grave(player), grave_position))
    return;
...
{
  if (!deck_player_is_valid(player) || !test)
    return 0;
   ...
   * that interfere with the graveyard. */
  int iids[DECK_MAX_ZONE_CARDS];
  int i, j, num_iids = 0;

  int* grave = graveyard_ptr_mutable[player];
  int* source = graveyard_source[player];
  for (i = j = 0; i < DECK_MAX_ZONE_CARDS && grave[i] != -1; ++i)
...
int find_in_owners_graveyard(int player, int card, int* owner, int* position)
{
  ...
  *owner = get_owner(player, card);
  ...
  int i;
  for (i = DECK_MAX_ZONE_CARDS - 1; i >= 0; --i)
...
{
  if (!deck_player_is_valid(player) || source == -1)
    return -1;

  if (deck_zone_position_is_valid(position_hint))
    {
  ...
  int i;
  for (i = 0; i < DECK_MAX_ZONE_CARDS; ++i)
  ...
  if (when_a_card_is_put_into_graveyard_from_anywhere_but_library_do_something(player, card, event, t_player, RESOLVE_TRIGGER_MANDATORY, NULL)
      && gy_from_anywhere_pos != -1)
    {
      ...
      if (gy_from_anywhere_source != -1
          && ((type > 0 && is_what(-1, trigger_cause, type))
              || (this_test != NULL && new_make_test(p, trigger_cause, -1, this_test))))
        {
      ...
      if (i == player || player == ANYBODY)
        {
          ...
          while (count < DECK_MAX_ZONE_CARDS && grave[count] != -1)
            {
