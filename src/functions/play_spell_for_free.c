...
/*
 * Free-spell and copy-spell helpers.
 *
 * This file handles effects that move cards between zones and then cast them
 * without paying normal mana costs.  Most of these paths temporarily mark a card
 * as "free" so compute_and_check_casting_cost() can preserve all other cost
 * modification logic while skipping the printed mana cost.
 *
 * The code deliberately works with real Shandalar card slots, internal card ids,
 * csv ids, and graveyard/exile positions, all represented as ints.  Keep the
 * guardrails local and explicit so those meanings do not blur together.
 */
...
static void clear_free_spell_marker(void)
{
	free_spell_player = -1;
	free_spell_card = -1;
}
...
int not_played_from_hand(int player, int card)
{
	...
	return check_special_flags(player, card, SF_NOT_CAST | SF_PLAYED_FROM_GRAVE | SF_PLAYED_FROM_DECK | SF_PLAYED_FROM_EXILE) ? 1 : 0;
}

static int real_copy_spell(int player, int internal_id, int special_flags, int special_flags3, card_instance_t* copied)
{
	if (!is_real_player(player) || internal_id < 0) {
		return -1;
	}
	...
	if (copied) {
		/*
		 * A copied spell should preserve the source spell's selected X value,
		 * alternate legacy text flags, and target array.  This intentionally
		 * copies the target storage rather than asking the player to target again.
		 */
	...
	if (put_card_on_stack(player, card_added, 0)) {
		...
		/*
		 * Directly putting a spell onto the stack this way does not really work,
		 * but old callers appear to rely on these globals being set.
		 */
...
{
	/*
	 * put_card_on_stack() clears 0x78676C before the engine consumes it.  This
	 * temporary marker lets the controlled callback restore enough context for X
	 * spells cast without paying their mana costs.
	 */
	target_t old_hack_val = totally_bletcherous_hack_dont_reset_x_value;
	...
	totally_bletcherous_hack_dont_reset_x_value = old_hack_val;
...
static int real_play_spell_for_free(int player, int card, int special_flags)
{
	...
	int internal_id = instance->internal_card_id;
	if (internal_id < 0) {
		return 0;
	}
	...
	if (totally_bletcherous_hack_put_card_on_stack0_without_resetting_x_value(player, card)) {
		...
		/*
		 * This does not actually put the spell on the stack, but the legacy code
		 * exposes this state to surrounding engine logic.
		 */
		card_on_stack = card;
		card_on_stack_controller = player;
		...
		if (card_d->type & TYPE_LAND) {
			...
			/*
			 * This remains deliberately conservative.  The old code increments
			 * only part of the storm bookkeeping for some free-cast cases, and
			 * changing that would be a gameplay behavior change rather than a
			 * guardrail.
			 */
			int mode = 0;
...
{
	if (!is_real_player(player) || !is_real_player(t_player) || deck_pos < 0 || deck_pos >= MAX_ZONE_CARDS) {
		return -1;
	}
...
{
	if (!is_real_player(player) || !is_real_player(t_player) || g_pos < 0 || g_pos >= MAX_ZONE_CARDS) {
		return -1;
	}
...
{
	if (!is_real_player(player) || !is_real_player(t_player) || csvid < 0) {
		return -1;
	}
	...
	if (iid >= 0 && can_legally_play_iid(player, iid)) {
		card_added = add_card_to_hand(player, iid);
		if (card_added == -1) {
			return -1;
		}
...
/*
 * Needed for counterspells to correctly allow the Commander to be put into the
 * command zone, and for cards that need to know how much mana was paid to play
 * them.
 */
int play_card_in_exile_for_free_commander(int player, int t_player, int csvid, int mana_paid_for_this)
{
	return play_card_in_exile_for_free_impl(player, t_player, csvid, SF2_COMMANDER, mana_paid_for_this);
}
...
{
	/*
	 * Madness is not literally "play from exile" here; this flag preserves the
	 * engine behaviors that care about not being played from hand.
	 */
	if (is_card_ref(player, card)) {
...
void opponent_plays_card_in_your_hand_for_free(int player, int card)
{
	...
	int card_added = add_card_to_hand(opponent, iid);
	if (card_added == -1) {
		return;
	}
	...
	if (player == AI) {
		add_state(opponent, card_added, STATE_OWNED_BY_OPPONENT);
	} else {
		remove_state(opponent, card_added, STATE_OWNED_BY_OPPONENT);
	}

	real_play_spell_for_free(opponent, card_added, 0);
}

int put_into_play(int player, int card)
{
	/*
	 * This should be used when a card is directly put into play from a zone.
	 * put_into_play_exe is reserved for lower-level executable behavior.
	 */
	if (!is_card_ref(player, card)) {
		return -1;
	}
...
int compute_and_check_casting_cost(int player, int unused, int card)
{
	/*
	 * 200bb14 / 0x402930.
	 * Parameter unused is always the same as player when called from the exe.
	 */
	(void)unused;

	int result = 0;

	int old_73825c = EXE_DWORD(0x73825C);
...
#undef COMPUTE_COLOR
	...
	if (cd->type & TYPE_ARTIFACT) {
		result = has_mana_multi_a(player,
								  cost_xbugrwaU[COLOR_COLORLESS],
								  cost_xbugrwaU[COLOR_BLACK],
								  cost_xbugrwaU[COLOR_BLUE],
								  cost_xbugrwaU[COLOR_GREEN],
								  cost_xbugrwaU[COLOR_RED],
								  cost_xbugrwaU[COLOR_WHITE]);
	} else {
...
static int effect_may_play_card_from_exile(int player, int card, event_t event)
{
	...
	if (event == EVENT_CAN_ACTIVATE && instance->targets[1].card == 42) {
		return 0;
	}
	...
	if (event == EVENT_RESOLVE_ACTIVATION) {
		...
		if (iid < 0) {
			return can_look_at_card;
		}

		can_play = can_legally_play_iid_now(player, iid, event);
	...
	if (event == EVENT_CAN_ACTIVATE) {
		return can_play ? can_play : can_look_at_card;
	}
	...
	if (event == EVENT_ACTIVATE) {
		if (can_look_at_card && DIALOG(player, card, event, DLG_NO_STORAGE, DLG_NO_CANCEL, DLG_FULLCARD_CSVID(instance->display_pic_csv_id),
									   ...
									   "Look at card", 1, -1) == 2) {
			show_deck(player, &iid, 1, "Face-down card in exile", 0, (int)EXE_STR(0x78f0a8));
			cancel = 1;
			return 0;
		}
...
int create_may_play_card_from_exile_effect(int player, int card, int whose_exile_zone, int csvid, mpcfe_mode_t mode)
{
	...
	if (leg != -1) {
		...
		if (!(mode & MPCFE_FACE_DOWN)) {
			create_card_name_legacy(player, leg, csvid);
		}
	}
...
{
	/*
	 * targets[0].card: iid of chosen card
	 * targets[1].player: graveyard_source value
	 * targets[1].card: tentative graveyard position
	 * targets[2].player: flashback_legacy_t flags
	 */

	if (event == EVENT_CLEANUP
		|| (event == EVENT_STATIC_EFFECTS && get_card_instance(player, card)->targets[1].card == -1)) {
		kill_card(player, card, KILL_REMOVE);
	}
...
int can_activate_to_play_cards_from_graveyard(int player, int card, event_t event, type_t types)
{
	if (event == EVENT_CAN_ACTIVATE) {
		...
		for (i = 0; grave[i] != -1; ++i) {
			if ((cards_data[grave[i]].type & types)
	...
	if (event == EVENT_ACTIVATE) {
		const int* grave = get_grave(player);
		int iids[MAX_ZONE_CARDS], positions[MAX_ZONE_CARDS];
		int i, num_cards = 0;

		for (i = 0; grave[i] != -1 && num_cards < MAX_ZONE_CARDS; ++i) {
			if ((cards_data[grave[i]].type & types)
				&& has_mana_to_cast_iid(player, event, grave[i])
...
{
	if (player < 0 || card < 0 || player > 1 || card >= 150) {
		return 0;
	}
	...
	if ((csvid == CARD_ID_VESUVAN_DOPPELGANGER || csvid == 39)
	...
	dispatch_trigger2(current_turn, TRIGGER_COMES_INTO_PLAY, EXE_STR(0x787108), 0, player, card);
...
{
	int playable[2][MAX_CARD_SLOTS];
	int pc = 0;
	int i;

	for (i = 0; i < active_cards_count[1 - player] && pc < MAX_CARD_SLOTS; i++) {
