// -*- c-basic-offset:2 -*-
// Keyword abilities, and abilities handled analogously to them.

#include "manalink.h"

/*
 * This file is the keyword-ability glue layer.
 *
 * Most abilities here are not standalone rules implementations.  They cooperate
 * with the original executable's event loop by setting bits in card_instance_t,
 * creating small legacy effects, or nudging event_result during a narrow event.
 * Keep that choreography explicit: a compact helper is fine, but broad rewrites
 * tend to disturb timestamp order, AI assumptions, or the exe's targeting UI.
 */

enum {
  ABILITY_MAX_PLAYERS = 2,
  ABILITY_MAX_CARD_SLOTS = 150,

  /* targets[17].card stores three independent saturating counters.  Multiple
   * effects can overlap, so these are counters rather than booleans. */
  ABILITY_LOCK_NONMANA_ACTIVATED = 0x00F,
  ABILITY_LOCK_ALL_ACTIVATED = 0x0F0,
  ABILITY_LOCK_HUMILIATION = 0xF00,
  ABILITY_LOCK_ANY_ACTIVATED_OR_HUMILIATION = 0xFFF
};
...
{
  return card >= 0 && card < ABILITY_MAX_CARD_SLOTS;
}
...
static int active_count_for_player(int player)
{
  return is_real_player(player) ? MIN(active_cards_count[player], ABILITY_MAX_CARD_SLOTS) : 0;
}

static int selector_matches_player(int selector, int player)
{
  return selector == ANYBODY || selector == player;
}

static card_instance_t* safe_get_instance(int player, int card)
{
...
{
  return is_card_ref(player, card) ? in_play(player, card) : NULL;
}

static void ensure_special_ability_storage(card_instance_t* instance)
{
  if (instance && instance->targets[16].card < 0)
    instance->targets[16].card = 0;
}

static void add_special_ability_by_instance(card_instance_t* instance, int ability)
{
  if (!instance)
    return;

  ensure_special_ability_storage(instance);
  instance->targets[16].card |= ability;
}

static void remove_special_ability_by_instance(card_instance_t* instance, int ability)
{
  if (!instance || instance->targets[16].card < 0)
    return;
...
static void adjust_targets17_counter(card_instance_t* instance, uint32_t mask, uint32_t step, int adding)
{
  if (!instance)
    return;
  ...
  uint32_t value = instance->targets[17].card & mask;
  if (adding)
    {
      if (value != mask)
        value += step;
  ...
  instance->targets[17].card &= ~mask;
  instance->targets[17].card |= value;
}
...
/*
 * targets[17].card carries ability-removal state:
 *   bits 0-3:   non-mana activated abilities are disabled.
 *   bits 4-7:   all activated abilities are disabled.
 *   bits 8-11:  all abilities are removed, also called humiliation.
 *
 * The exe has only a few spare fields, so this packed storage is intentionally
 * ugly.  The important invariant is that each nibble behaves like a saturating
 * reference count: adding the first effect enables the lock, removing the last
 * effect clears it, and overlapping effects do not stomp each other.
 */

int can_use_activated_abilities(int player, int card)
{
  return can_use_activated_abilities_by_instance(safe_get_instance(player, card));
}

int can_use_activated_abilities_by_instance(card_instance_t* instance)
{
  return instance && (instance->targets[17].card <= 0 || !(instance->targets[17].card & ABILITY_LOCK_ANY_ACTIVATED_OR_HUMILIATION));
}

int can_produce_mana(int player, int card)
{
  card_instance_t* instance = safe_get_instance(player, card);
  if (!instance)
    return 0;

  /* Non-mana activated abilities can be disabled without stopping mana abilities.
   * All-activated and humiliation counters both shut mana production off. */
  if (instance->targets[17].card > 0 && (instance->targets[17].card & (ABILITY_LOCK_ALL_ACTIVATED | ABILITY_LOCK_HUMILIATION)))
    return 0;

  if (player == AI
      && instance->targets[14].card != -1
      && (instance->targets[14].card & SF_MANA_PRODUCER_DISABLED_FOR_AI))
    return 0;
...
{
  return instance && (instance->targets[17].card > 0 && (instance->targets[17].card & ABILITY_LOCK_HUMILIATION));
}

int is_humiliated(int player, int card)
{
  return is_humiliated_by_instance(safe_get_instance(player, card));
}

void disable_nonmana_activated_abilities(int player, int card, int mode)
{
  adjust_targets17_counter(safe_get_instance(player, card), ABILITY_LOCK_NONMANA_ACTIVATED, 0x001, mode);
}

void disable_all_activated_abilities(int player, int card, int mode)
{
  adjust_targets17_counter(safe_get_instance(player, card), ABILITY_LOCK_ALL_ACTIVATED, 0x010, mode);
}

static void real_humiliation(int t_player, int t_card, int mode)
{
  adjust_targets17_counter(safe_get_instance(t_player, t_card), ABILITY_LOCK_HUMILIATION, 0x100, mode);
}

/*****************************
* Power/toughness set layers *
*****************************/

/*
 * Set-power/toughness effects behave like timestamped layers.  Several effects
 * can be attached to the same permanent, but only the newest controlled effect
 * contributes to EVENT_POWER and EVENT_TOUGHNESS.  When the newest effect ends,
 * the next newest one is reactivated.
 */
...
int set_pt_legacy(int player, int card, event_t event)
{
  ...
  if (is_card_ref(t_player, t_card) && check_status(player, card, STATUS_CONTROLLED))
    {
      if (event == EVENT_POWER && affect_me(t_player, t_card))
        event_result += instance->counter_power;
      if (event == EVENT_TOUGHNESS && affect_me(t_player, t_card))
  ...
  if (instance->targets[1].player > -1)
    {
      if ((instance->targets[1].player & 1) && event == EVENT_CLEANUP)
        {
          reactivate_the_most_recent_pt_setting_effects_attached_to_me(t_player, t_card);
      ...
      if ((instance->targets[1].player & 4)
          && current_turn == t_player
...
{
  card_instance_t* inst = safe_get_instance(player, card);
  return inst
    && is_what(player, card, TYPE_EFFECT)
    && (inst->info_slot == (int)set_pt_legacy || inst->display_pic_csv_id == CARD_ID_GIGANTOPLASM);
}

static int effect_on_all_pt_setting_effects_attached_to_me(int player, int card, int effect)
{
  /* effect =  0: check only.
   * effect =  1: enable the newest inactive set_pt_legacy.
   * effect = -1: disable every set_pt_legacy attached to this permanent. */
  int result = 0;
  ...
  for (p = 0; p <= 1; ++p)
    for (c = active_count_for_player(p) - 1; c >= 0; --c)
      if ((inst = in_play(p, c))
          && inst->damage_target_player == player
          && inst->damage_target_card == card
          && is_pt_effect(p, c))
        {
          if (effect == 0)
            result = 1;
          else if (effect == 1 && !check_status(p, c, STATUS_CONTROLLED))
            {
...
int set_pt_and_abilities_until_eot(int player, int card, int t_player, int t_card, int pow, int tou, int key, int s_key, int mode)
{
  ...
  if (t_player != ANYBODY && t_card != -1)
    {
      ...
      if (key || s_key)
        {
          int legacy = pump_ability_until_eot(player, card, t_player, t_card, 0, 0, key, s_key);
  ...
  int p;
  for (p = 0; p <= 1; ++p)
    if (selector_matches_player(t_player, p))
      {
        int c;
        for (c = active_count_for_player(p) - 1; c >= 0; --c)
          if (in_play(p, c) && is_what(p, c, TYPE_CREATURE))
            {
              if ((mode & 1) && !(is_attacking(p, c) && is_unblocked(p, c)))
                continue;

              real_set_pt(player, card, p, c, pow, tou, 1);
              if (key || s_key)
                {
                  int legacy = pump_ability_until_eot(player, card, p, c, 0, 0, key, s_key);
                  add_status(player, legacy, STATUS_INVISIBLE_FX);
                }
            }
      }
              ...
              switch (event)
                {
                  ...
                  case EVENT_ABILITIES:
                    /* Clear normal keyword abilities while preserving recalc flags.
                     * Special abilities are stored separately, so clear the visible
                     * icon bits as well. */
                    event_result &= (KEYWORD_RECALC_CHANGE_TYPE | KEYWORD_RECALC_TOUGHNESS | KEYWORD_RECALC_POWER
                                     | KEYWORD_RECALC_ABILITIES | KEYWORD_RECALC_SET_COLOR);
                    card_instance_t* aff = get_card_instance(p, c);
                    aff->state &= ~STATE_VIGILANCE;
                    if (aff->targets[16].card != -1)
                      aff->targets[16].card &= ~(SP_KEYWORD_FEAR | SP_KEYWORD_INTIMIDATE | SP_KEYWORD_UNBLOCKABLE
                                                 | SP_KEYWORD_DEATHTOUCH | SP_KEYWORD_LIFELINK | SP_KEYWORD_INDESTRUCTIBLE
                                                 | SP_KEYWORD_SHADOW | SP_KEYWORD_HEXPROOF | SP_KEYWORD_HORSEMANSHIP
...
int humiliate_and_set_pt_abilities(int player, int card, int t_player, int t_card, int mode, test_definition_t *hc)
{
   ...
   * mode = 2: remove all abilities and attach a legacy that sets power/toughness, keywords, and optionally color/subtype.
   * mode = 4: humiliation legacy ends at cleanup. */
  if (!is_card_ref(t_player, t_card))
    return -1;
      ...
      if (mode & ((1 << 1) | (1 << 2)))
        {
          ...
          int p, c;
          for (p = 0; p <= 1; ++p)
            for (c = 0; c < active_count_for_player(p); ++c)
              {
...
/*
 * Special keywords live in targets[16].card because the original keyword field
 * is already crowded.  Most of these functions only set a bit during
 * EVENT_ABILITIES.  Effects that need other timing, such as lifelink during
 * damage, are interpreted later from the stored bit.
 */
...
void special_abilities(int player, int card, event_t event, int sp_abilities, int source_player, int source_card)
{
  ...
  if (sp_abilities & SP_KEYWORD_SHADOW_HOSER)
    add_special_ability_by_instance(instance, SP_KEYWORD_SHADOW_HOSER);
...
{                                                                       \
  if (event == EVENT_ABILITIES && affect_me(player, card))              \
    {                                                                   \
      ...
      if (!instance)                                                    \
        return;                                                         \
      add_special_ability_by_instance(instance, bit);                    \
    }                                                                   \
}
...
int indestructible_until_eot(int player, int card, event_t event)
{
  if (event == EVENT_ABILITIES)
    {
      card_instance_t* instance = get_card_instance(player, card);
      if (is_card_ref(instance->damage_target_player, instance->damage_target_card)
...
{
  if (is_card_ref(player, card) && in_play(player, card))
    {
      card_instance_t *instance = get_card_instance(player, card);
...
{
  /* A shield is a delayed regeneration.  It advertises regeneration for the AI,
   * then consumes itself as soon as the attached permanent is actually being
   * destroyed. */
  card_instance_t *instance = get_card_instance(player, card);

  if (is_card_ref(instance->targets[0].player, instance->targets[0].card))
    {
      ...
      static int regeneration_from_shield_in_progress = 0;
...
int regenerate_or_shield(int player, int card, int t_player, int t_card)
{
  ...
  if ((player == t_player && card == t_card) ? can_regenerate(t_player, t_card) : can_be_regenerated(t_player, t_card))
    {
...
int cannot_regenerate_until_eot(int player, int card, int t_player, int t_card)
{
  ...
  if (leg2 != -1)
    {
      /* The exe checks for a generic legacy with STATUS_CANNOT_REGENERATE attached
       * to the card, not just the status bit on the card itself. */
...
int attack_if_able(int player, int card, event_t event)
{
  if (event == EVENT_MUST_ATTACK && current_turn == player && !forbid_attack)
    {
      card_instance_t* attacker = safe_get_instance(player, card);
      if (attacker && !(attacker->state & STATE_UNKNOWN8000))
        {
...
{
  return can_block_me(player, card, can_block_target_player, can_block_target_card)
    ? NULL
    : EXE_STR(0x5b8580);  /* localized "Illegal block." */
}
...
int block_if_able(int player, int card, event_t event)
{
  ...
  add_special_ability_by_instance(instance, SP_KEYWORD_MUST_BLOCK);
...
const char* target_must_be_able_to_block_me(int who_chooses, int player, int card, int targeting_player, int targeting_card)
{
  ...
  return instance
    ...
    && is_legal_block(player, card, targeting_player, targeting_card)
    ? NULL
    : EXE_STR(0x5b8580);
}
...
static void lure_effect(int player, int card, event_t event, int luring_player, int luring_card, int all_must_block, int subtype, const char* prompt)
{
  /* Lure is handled as a mandatory-block trigger rather than as a passive
   * legality rule.  This lets multiple Lure-like effects coexist and lets the
   * appropriate player pick blockers one at a time. */
          ...
          if (ai_is_speculating == 1)
            *fullprompt = 0;
          else
            {
              ...
              int csvid = is_what(player, card, TYPE_EFFECT)
                ? get_card_instance(player, card)->display_pic_csv_id
                : get_id(player, card);
              scnprintf(fullprompt, 200, "%s: %s", cards_ptr[csvid]->full_name, prompt);
            }

          target_t tgt;
          int old_number_of_targets = luring->number_of_targets;
          trigger_condition = -1;
...
int target_must_block_me(int player, int card, int t_player, int t_card, int mode)
{
  ...
  if (legacy_card != -1)
    get_card_instance(player, legacy_card)->targets[2].player = mode;
  return legacy_card;
}
...
int check_for_flanking_removal(int player, int card)
{
  ...
  int c;
  for (c = 0; c < active_count_for_player(1-player); ++c)
  ...
  if (event == EVENT_ABILITIES && affect_me(t_player, t_card))
    {
      if (!check_for_flanking_removal(t_player, t_card))
        add_special_ability_by_instance(safe_get_instance(t_player, t_card), SP_KEYWORD_FLANKING);
    }

  if (event == EVENT_DECLARE_BLOCKERS
      && is_attacking(t_player, t_card)
      && check_for_special_ability(t_player, t_card, SP_KEYWORD_FLANKING))
    {
      char marked[2][151] = {{0}};
      mark_each_creature_blocking_me(t_player, t_card, marked);
      int c;
      for (c = 0; c < active_count_for_player(1-current_turn); ++c)
...
int freeze_when_damage(int player, int card, event_t event)
{
  ...
  add_special_ability_by_instance(instance, SP_KEYWORD_FREEZE_WHEN_DAMAGE);
  ...
  if (event == EVENT_ABILITIES && affect_me(player, card))
    {
      ...
      add_special_ability_by_instance(instance, SP_KEYWORD_HASTE);
  ...
  if (count_minus1_minus1_counters(player, card) <= 0)
    {
      ...
      if (this_dies_trigger(player, card, event, RESOLVE_TRIGGER_MANDATORY)
        ...
        reanimate_permanent(owner, -1, owner, position, REANIMATE_MINUS1_MINUS1_COUNTER);
    }
}
  ...
  if (event == EVENT_ABILITIES && affect_me(player, card))
    {
      ...
      add_special_ability_by_instance(instance, SP_KEYWORD_VIGILANCE);
...
{
  if (card != -1)
    return is_card_ref(player, card) && (get_abilities(player, card, EVENT_ABILITIES, -1) & keyword) ? 1 : 0;

  int c;
  for (c = 0; c < active_count_for_player(player); ++c)
    if (in_play(player, c) && is_what(player, c, TYPE_CREATURE))
      if (get_abilities(player, c, EVENT_ABILITIES, -1) & keyword)
        return 1;
...
{
  if (!instance || is_humiliated_by_instance(instance))
    return 0;
  ...
  return instance->targets[16].card;
}

int get_special_abilities(int player, int card)
{
  return get_special_abilities_by_instance(safe_get_instance(player, card));
}

int check_for_special_ability(int player, int card, int keyword)
{
  return get_special_abilities_by_instance(safe_get_instance(player, card)) & keyword;
}

void remove_special_ability(int player, int card, int sa)
{
  remove_special_ability_by_instance(safe_get_instance(player, card), sa);
}
...
void set_special_abilities(int player, int card, int type, int value)
{
  if (card != -1)
    {
      add_special_ability_by_instance(safe_get_instance(player, card), value);
      return;
    }

  int p;
  for (p = 0; p <= 1; ++p)
    if (selector_matches_player(player, p))
      {
        int c;
        for (c = active_count_for_player(p) - 1; c >= 0; --c)
          if (in_play(p, c) && is_what(p, c, type))
            add_special_ability_by_instance(get_card_instance(p, c), value);
      }
}

/*******************************
* Temporary ability negation   *
*******************************/

/*
 * Ability-negation legacies use the same timestamp stack as set_pt_legacy: only
 * the newest controlled negation effect attached to a creature applies.  This
 * matters when one effect removes flying and a later one removes first strike;
 * when the later effect ends, the earlier one must become visible again.
 */
  ...
  if (event == EVENT_ABILITIES && check_status(player, card, STATUS_CONTROLLED))
    {
      int t_player = instance->damage_target_player;
      int t_card = instance->damage_target_card;
      if (is_card_ref(t_player, t_card) && affect_me(t_player, t_card))
        {
          if (instance->targets[1].player > 0)
            event_result &= ~instance->targets[1].player;
          if (instance->targets[1].card > 0)
            remove_special_ability(t_player, t_card, instance->targets[1].card);
        }
    }

  if (event == EVENT_CLEANUP && (instance->targets[2].player & 1))
    {
...
{
  /* effect =  0: check only.
   * effect =  1: enable the newest inactive control legacy.
   * effect = -1: disable every control legacy attached to this permanent. */
  int result = 0;
  int best_timestamp = 0;
  int best_player = -1;
  int best_card = -1;
  ...
  for (p = 0; p <= 1; ++p)
    for (c = active_count_for_player(p) - 1; c >= 0; --c)
      if ((inst = in_play(p, c))
          && inst->damage_target_player == player
          && inst->damage_target_card == card
          && is_what(p, c, TYPE_EFFECT)
          && inst->info_slot == (int)legacy_negate_ability_and_special_ability)
        {
          if (effect == 0)
            result = 1;
          else if (effect == 1 && !check_status(p, c, STATUS_CONTROLLED))
...
static int create_negate_ability_and_special_ability_until_eot(int player, int card, int t_player, int t_card, keyword_t keyword, int sp_ability)
{
  if (!is_card_ref(t_player, t_card))
    return -1;
  ...
  if (legacy != -1)
    {
      ...
      leg->targets[2].player = 1;  /* End at cleanup. */
    }
  return legacy;
}
...
{
  /* A single target returns its legacy.  Broad applications return -1 after
   * creating one legacy per affected creature. */
  if (t_card != -1)
    ...
    if (selector_matches_player(t_player, p))
      {
        ...
        for (c = active_count_for_player(p) - 1; c >= 0; --c)
          if (in_play(p, c) && is_what(p, c, TYPE_CREATURE))
            create_negate_ability_and_special_ability_until_eot(player, card, p, c, keyword, s_keyword);
      }
