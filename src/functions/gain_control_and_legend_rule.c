...
enum
{
	CONTROL_BATTLEFIELD_CARD_CAPACITY = 150,
	CONTROL_MAX_TARGETS = 18
};

static int control_player_is_valid(int player)
{
	return player >= HUMAN && player <= AI;
}

static int control_active_cards_count(int player)
{
	return control_player_is_valid(player) ? MIN(active_cards_count[player], CONTROL_BATTLEFIELD_CARD_CAPACITY) : 0;
}

static int control_valid_card_index(int player, int card)
{
	return control_player_is_valid(player) && card >= 0 && card < control_active_cards_count(player);
}

static int control_target_count(card_instance_t* instance)
{
	if (!instance || instance->number_of_targets <= 0){
		return 0;
	}
	return MIN(instance->number_of_targets, CONTROL_MAX_TARGETS);
}
...
int true_verify_legend_rule(int player, int card, int id){
	...
	int l_count = 0, c = 0;
	int active_count = control_active_cards_count(player);
...
// What this function do: 1-player gains control of (player, card)
	if( !control_valid_card_index(player, card) || !in_play(player, card) ){
		return -1;
	}

	card_instance_t *old_instance = get_card_instance(player, card);
	int old_iid = old_instance->internal_card_id;
	if( old_iid < 0 ){
		return -1;
	}

	int new_controller = 1 - player;
	int old_timestamp = old_instance->timestamp;
	int new_card = add_card_to_hand(new_controller, old_iid);
	if( new_card == -1 ){
		return -1;
	}

	card_instance_t *new_instance = get_card_instance(new_controller, new_card);
	memcpy(new_instance, old_instance, sizeof(card_instance_t));
	new_instance->state |= STATE_SUMMON_SICK;
	new_instance->state &= ~(STATE_ATTACKING | STATE_BLOCKING);

	timestamp_player[old_timestamp] = new_controller;
	timestamp_card[old_timestamp] = new_card;

	int p;
	for(p = HUMAN; p <= AI; ++p){
		int active_count = control_active_cards_count(p);
		int c;
		for(c = 0; c < active_count; ++c){
			...
			if( instance->damage_target_player == player && instance->damage_target_card == card ){
				instance->damage_target_player = new_controller;
				instance->damage_target_card = new_card;
			}
			if( instance->damage_source_player == player && instance->damage_source_card == card ){
				instance->damage_source_player = new_controller;
				instance->damage_source_card = new_card;
			}
			int targets = control_target_count(instance);
			int i;
			for(i = 0; i < targets; ++i){
				if( instance->targets[i].player == player && instance->targets[i].card == card ){
					instance->targets[i].player = new_controller;
					instance->targets[i].card = new_card;
				}
			}
		}
	}

	int typ = cards_data[old_iid].type;
	if (typ & TYPE_CREATURE){
		++creature_count[new_controller];
	}
	if (typ & TYPE_ARTIFACT){
		++artifact_count[new_controller];
	}
	if (typ & TYPE_ENCHANTMENT){
		++enchantment_count[new_controller];
	}
	types_of_cards_on_bf[new_controller] |= typ;
	redraw_libraries();
	old_instance->token_status |= STATUS_CANNOT_LEAVE_PLAY | STATUS_OBLITERATED;
	kill_card(player, card, KILL_REMOVE);
	if (BYTE3(event_flags) & 0x10){                // EA_CONTROLLED
		dispatch_event(new_controller, new_card, EVENT_CARDCONTROLLED);
	}
	return new_card;
}
...
static int generic_control_legacy(int player, int card, event_t event){
	...
	if( instance->damage_target_player > -1 ){
		...
		if( !control_valid_card_index(t_player, t_card) || !in_play(t_player, t_card) ){
			kill_card(player, card, KILL_REMOVE);
			return 0;
		}
		...
		if( mode && (mode & (GCUS_CONTROLLED | GCUS_TAPPED | GCUS_BURY_IF_TAPPED_OR_LEAVE_PLAY)) ){
			...
			(trigger_condition == TRIGGER_LEAVE_PLAY && affect_me(player, card)))
			{
				...
				if (s_player >= 0 && s_card >= 0 && other_leaves_play(player, card, s_player, s_card, event))
				{
				...
				if (event == EVENT_CARDCONTROLLED && s_player >= 0 && s_card >= 0 && affect_me(s_player, s_card) && (mode & GCUS_CONTROLLED))
				{
					re_enable_the_most_recent_control_effect_attached_to_me(t_player, t_card);
					if( check_state(player, card, STATE_POWER_STRUGGLE) && check_status(player, card, STATUS_CONTROLLED) ){
						real_switch_control(t_player, t_card);
				...
				if (event == EVENT_STATIC_EFFECTS && control_valid_card_index(s_player, s_card) && in_play(s_player, s_card) &&
...
// effect = -1 -> disable all "control legacies"
	...
	int p, c;
	for (p = HUMAN; p <= AI; ++p){
		for (c = control_active_cards_count(p)-1; c > -1; c--){
...
void exchange_control_of_target_permanents(int player, int card, int player1, int card1, int player2, int card2)
{
  if (player1 != player2)
	{
	  if (!control_valid_card_index(player1, card1) || !in_play(player1, card1)
		  || !control_valid_card_index(player2, card2) || !in_play(player2, card2)
		  || check_for_guardian_beast_protection(player1, card1)
		  || check_for_guardian_beast_protection(player2, card2))
		return;
...
// Mode:	see 'gcus_t' in 'defs.h'.
	...
	if( result > -1 ){
		...
		if( legacy_card > -1 ){
			...
			legacy_instance->targets[3].card = card >= 0 ? get_id(player, card) : -1;
		}
	}
	return legacy_card;
}
...
{
	if( !control_valid_card_index(tar_player, tar_card) || !in_play(tar_player, tar_card) ){
		return -1;
	}
...
int effect_act_of_treason_and_modify_pt_or_abilities(int player, int card, int t_player, int t_card, int pow, int tou, int key, int s_key){
	...
	if (in_play(t_player, t_card)){
		...
		if( player != t_player ){
			legacy_card = gain_control_until_clause(player, card, t_player, t_card, 1, pow, tou, key, s_key | SP_KEYWORD_HASTE);
		}
		else{
  ...
  if (in_play(t_player, t_card))
	{
	  ...
	  if (!control_valid_card_index(source_player, source_card)
		  || !in_play(source_player, source_card)
