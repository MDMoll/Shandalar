...
enum
{
  HCT_BATTLEFIELD_CARD_CAPACITY = 150,
  HCT_MAX_ZONE_CARDS = 500,
  HCT_CARD_TYPE_SLOTS = 6,
  HCT_CHOICE_SUBTYPES = 6
};

static int hct_player_is_valid(int player)
{
  return player >= HUMAN && player <= AI;
}

static int hct_active_cards_count(int player)
{
  return hct_player_is_valid(player) ? MIN(active_cards_count[player], HCT_BATTLEFIELD_CARD_CAPACITY) : 0;
}

static int hct_valid_card_index(int player, int card)
{
  return hct_player_is_valid(player) && card >= 0 && card < hct_active_cards_count(player);
}

static int hct_get_iid(int player, int card)
{
  if (player == -1)
    return card;

  if (!hct_valid_card_index(player, card))
    return -1;

  card_instance_t* inst = get_card_instance(player, card);
  int iid = inst->internal_card_id;
  if (iid < 0)
    iid = inst->backup_internal_card_id;
  if (iid < 0)
    iid = inst->original_internal_card_id;

  return iid;
}

static int hct_csv_id_is_valid(int id)
{
  return id >= 0 && id < available_slots && cards_ptr[id];
}

static int is_creature_type(int new_type);
const char* raw_get_subtype_text(subtype_t subtype);

static void hct_add_choice_subtype(int subs[HCT_CHOICE_SUBTYPES], int* sc, subtype_t subtype)
{
  if (!is_creature_type(subtype) || *sc >= HCT_CHOICE_SUBTYPES)
    return;

  int i;
  for (i = 0; i < *sc; ++i)
    if ((subtype_t)subs[i] == subtype)
      return;
...
{
  if (hcs >= 0 && hcs <= HARDCODED_SUBTYPE_VIASHINO)
...
static int has_new_types(int id, int subtype){
  ...
  if( subtype == -1 || !hct_csv_id_is_valid(id) ){
    return 0;
  }
  ...
  int i;
  for (i = 0; i < HCT_CARD_TYPE_SLOTS; ++i){
    if (c->types[i] == subtype){
      return 1;
    }
  }
...
static int is_changeling(int player, int card){
  ...
  if( player != -1 && (instance = in_play(player, card)) ){
    ...
    int iid = hct_get_iid(player, card);
    if( iid >= 0 && (cards_data[iid].static_ability & KEYWORD_PROT_INTERRUPTS) ){
      return 1;
    }
  }
  return 0;
...
static int force_a_subtype_until_eot_legacy(int player, int card, event_t event){
  ...
  if( eot_trigger(player, card, event) ){
    if( hct_valid_card_index(instance->targets[0].player, instance->targets[0].card) ){
...
int force_a_subtype_until_eot(int player, int card, int t_player, int t_card, subtype_t subtype){
  if( !hct_valid_card_index(t_player, t_card) ){
    return -1;
  }
...
int has_subtype_by_id(int id, subtype_t subtype){
  ...
  if( subtype <= 0 || !hct_csv_id_is_valid(id) ){
    return 0;
  }
  ...
  if( subtype == SUBTYPE_LEGEND ){
    if( cards_data[iid].subtype == 0x0e || cards_data[iid].subtype == 0x0f ){
      return 1;
    }
  }

  if( is_creature_type(subtype) && is_changeling(-1, iid) ){
    return 1;
  }
  ...
  if (subtype == SUBTYPE_AURA_CREATURE){
    ...
    return ((cp->subtype1 == HARDCODED_SUBTYPE_ARTIFACT_CREATURE_OR_AURA_MOSTLY_WITH_ENCHANT_CREATURE
             || cp->subtype2 == HARDCODED_SUBTYPE_ARTIFACT_CREATURE_OR_AURA_MOSTLY_WITH_ENCHANT_CREATURE)
...
int has_subtype(int player, int card, subtype_t subtype){
  ...
  if( player == -1 ){
    int iid = hct_get_iid(player, card);
    return iid >= 0 ? has_subtype_by_id(cards_data[iid].id, subtype) : 0;
  }
  ...
  if (is_in_play){
    ...
    if (forced == (unsigned int)subtype){
      return 1;
    }
    ...
    if (forced == (unsigned int)subtype || (unsigned int)get_added_subtype(player, card) == (unsigned int)subtype){
      return 1;
    }
    if (forced){
      return 0;
  ...
  if( creature_type ){
    ...
    if( is_in_play && id == CARD_ID_DUPLICANT ){
      int i;
      for(i=2; i<6; i++){
        if( (unsigned int)instance->targets[i].player == (unsigned int)subtype ){
          return 1;
        }
      }
    }
...
int shares_creature_subtype(int p0, int c0, int p1, int c1)
{
  ...
  for (i = 0; i < 2; ++i){
    if (p[i] != -1 && !hct_valid_card_index(p[i], c[i]))
      return 0;
  ...
  for (i = 0; i < 2; ++i)
    {
      ...
      int iid = hct_get_iid(p[i], c[i]);
      if (iid < 0)
        return 0;

      int csvid = cards_data[iid].id;
      if (!hct_csv_id_is_valid(csvid))
        return 0;

      card_ptr_t* cp = cards_ptr[csvid];
      for (j = 1; j < HCT_CARD_TYPE_SLOTS; ++j) // types[0] is always either a type or supertype; types[6] is unused
...
static int get_most_common_subtype_in_zone(const int *zone){
  ...
  int i, count;
  for (count = 0; count < HCT_MAX_ZONE_CARDS && zone[count] != -1; ++count){
      if( is_what(-1, zone[count], TYPE_CREATURE) ){
        int id = cards_data[zone[count]].id;
        if( !is_changeling(-1, zone[count]) && hct_csv_id_is_valid(id) ){
          card_ptr_t* c = cards_ptr[id];
          for (i = 1; i < HCT_CARD_TYPE_SLOTS; ++i){ // [0] is always either a type or supertype; [6] is unused
...
static int get_most_common_subtype_in_deck(int t_player){
  return hct_player_is_valid(t_player) ? get_most_common_subtype_in_zone(deck_ptr[t_player]) : -1;
}
...
static int select_a_subtype_impl(int player, int card, int who_chooses, int t_player, const int *zone, int show_legacy){
  ...
  td.allow_cancel = who_chooses == HUMAN;
 ...
 restart:;
  ...
  int sc = 0;
  char b1[500];
  int p1 = scnprintf(b1, 500, " Auto\n Choose from a card list\n");
  int can_choose_from_creature = can_target(&td);
  if( can_choose_from_creature ){
    choice = 2;
    p1 += scnprintf(b1+p1, 500-p1, " Choose from a creature");
  }
  if( who_chooses != AI ){
    choice = do_dialog(who_chooses, player, card, -1, -1, b1, can_choose_from_creature ? 2 : 0);
  }

  int csvid = -1;
  int subs[HCT_CHOICE_SUBTYPES] = {-1, -1, -1, -1, -1, -1};
  if( choice == 0 ){
    if( zone == NULL ){
      ...
      if( result < 1 ){
        result = internal_rand(SUBTYPE_MAX_USED_CREATURE_SUBTYPE - SUBTYPE_MIN_CREATURE_SUBTYPE + 1) + SUBTYPE_MIN_CREATURE_SUBTYPE;
      }
    }
    else{
      result = get_most_common_subtype_in_zone(zone);
      ...
      if( forced > 0 ){
        ...
        if( forced == SUBTYPE_ALL_CREATURES ){
          result = get_most_common_subtype_in_deck(t_player);
        }
        else{
          hct_add_choice_subtype(subs, &sc, forced);
          if( added > 0 && added != forced ){
            hct_add_choice_subtype(subs, &sc, added);
          }
        }
      }
        ...
        if( count_counters(instance->targets[0].player, instance->targets[0].card, COUNTER_GOLD) &&
          ...
          hct_add_choice_subtype(subs, &sc, SUBTYPE_WALL);
        }
        if( csvid == CARD_ID_FIGURE_OF_DESTINY ){
          if( chosen->targets[9].player > 0 ){
            if( chosen->targets[9].player & 1 ){
              hct_add_choice_subtype(subs, &sc, SUBTYPE_SPIRIT);
            }
            if( chosen->targets[9].player & 2 ){
              hct_add_choice_subtype(subs, &sc, SUBTYPE_WARRIOR);
            }
            if( chosen->targets[9].player == 3 ){
              hct_add_choice_subtype(subs, &sc, SUBTYPE_AVATAR);
            }
          }
        }
        if( csvid == CARD_ID_WARDEN_OF_THE_FIRST_TREE ){
          if( chosen->targets[9].player > 0 ){
            if( chosen->targets[9].player & 1 ){
              hct_add_choice_subtype(subs, &sc, SUBTYPE_WARRIOR);
            }
        ...
        if( csvid == CARD_ID_AGELESS_SENTINELS ){
          if( chosen->targets[5].player == 66 ){
            hct_add_choice_subtype(subs, &sc, SUBTYPE_BIRD);
            hct_add_choice_subtype(subs, &sc, SUBTYPE_GIANT);
          }
        }
        if( csvid == CARD_ID_DUPLICANT ){
          int j = 2;
          while( sc < HCT_CHOICE_SUBTYPES && j < 6 ){
            hct_add_choice_subtype(subs, &sc, chosen->targets[j].player);
            j++;
          }
        }
      }
  ...
  if( result == -1 ){
    ...
    if( hct_csv_id_is_valid(csvid) ){
      ...
      int i;
      for (i = 1; i < HCT_CARD_TYPE_SLOTS && sc < HCT_CHOICE_SUBTYPES; ++i){ // [0] is always either a type or supertype; [6] is unused
        hct_add_choice_subtype(subs, &sc, c->types[i]);
      }
    }
...
void protection_from_subtype(int player, int card, event_t event, int subt){
  ...
  if( event == EVENT_PREVENT_DAMAGE ){
    ...
    if( source->internal_card_id == damage_card && source->info_slot > 0 &&
      ...
      hct_valid_card_index(source->damage_source_player, source->damage_source_card) &&
...
int count_untapped_nonsick_subtype(int what_player, int type, int subtype){
  ...
  if( what_player == ANYBODY ){
     int i;
     for(i = 0; i < 2; i++){
       int count = hct_active_cards_count(i)-1;
       while( count > -1 ){
  ...
  else if ( what_player == AI || what_player == HUMAN){
       int count = hct_active_cards_count(what_player)-1;
       while( count > -1 ){
...
int is_subtype_in_hand(int player, int subtype){
  ...
  if( !hct_player_is_valid(player) ){
    return 0;
  }

  int i;
  int active_count = hct_active_cards_count(player);
...
int is_subtype_dead(int player, int subtype1, int subtype2, int mode){
    ...
    for(p=0;p<2;p++){
      if( p == player || player == ANYBODY ){
        int c=0;
        int active_count = hct_active_cards_count(p);
...
int subtype_deals_damage(int player, int card, event_t event, int controller, int subtype, int mode){
  ...
  if (event == EVENT_DEAL_DAMAGE){
    ...
    if (damage->internal_card_id == damage_card
      && hct_valid_card_index(damage->damage_source_player, damage->damage_source_card)
      && in_play(damage->damage_source_player, damage->damage_source_card)
      && (subtype == -1 ? is_what(damage->damage_source_player, damage->damage_source_card, TYPE_CREATURE)
                        : has_subtype(damage->damage_source_player, damage->damage_source_card, subtype))){
      int good = 0;
      if( damage->damage_source_player == controller || controller == ANYBODY ){
...
static char* hct_strdup_text_line(const char* s)
{
  return strdup(s ? s : "");
}
...
void init_subtype_text(void)
{
  ...
  for (i = 0; i <= (SUBTYPE_MAX_USED_CREATURE_SUBTYPE & 0xFFF); ++i)
    strs_creature_type[i] = hct_strdup_text_line(text_lines[i]);
  ...
  switch (subtype & 0xF000)
    {
      case 0: // Creature subtypes - note SUBTYPE_MIN_CREATURE_SUBTYPE is 1, not 0, unlike the other MINs
        if (subtype <= SUBTYPE_MAX_USED_CREATURE_SUBTYPE)
          return hct_subtype_text_or_empty(strs_creature_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_TYPE:
        if (subtype <= SUBTYPE_MAX_USED_TYPE)
          return hct_subtype_text_or_empty(strs_type_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_SUPERTYPE:
        if (subtype <= SUBTYPE_MAX_USED_SUPERTYPE)
          return hct_subtype_text_or_empty(strs_supertype_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_ARTIFACT_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_ARTIFACT_SUBTYPE)
          return hct_subtype_text_or_empty(strs_artifact_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_ENCHANTMENT_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_ENCHANTMENT_SUBTYPE)
          return hct_subtype_text_or_empty(strs_enchantment_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_LAND_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_LAND_SUBTYPE)
          return hct_subtype_text_or_empty(strs_land_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_INSTANT_SORCERY_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_INSTANT_SORCERY_SUBTYPE)
          return hct_subtype_text_or_empty(strs_instant_sorcery_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_PLANESWALKER_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_PLANESWALKER_SUBTYPE)
          return hct_subtype_text_or_empty(strs_planeswalker_type, subtype & 0xFFF);
        break;

      case SUBTYPE_MIN_PLANE_SUBTYPE:
        if (subtype <= SUBTYPE_MAX_USED_PLANE_SUBTYPE)
          return hct_subtype_text_or_empty(strs_plane_type, subtype & 0xFFF);
        break;
    }

  return strs_creature_type[0] ? strs_creature_type[0] : ""; // "None"
}
          ...
          if (*p == 'a')
            {
              ...
              if (raw_txt[0] && strchr("aeiouAEIOU", raw_txt[0]) && q < end)
                *q++ = 'n';
...
fx_change_types(int player, int card, event_t event)
{
  ...
  if (event == EVENT_GRAVEYARD_FROM_PLAY && affect_me(player, card))
    {
      ...
      if (hct_valid_card_index(was_attached_to_player, was_attached_to_card)
          && in_play(was_attached_to_player, was_attached_to_card))
        {
  ...
  if ((event == EVENT_EFFECT_CREATED
       ...
       && hct_valid_card_index(inst->damage_target_player, inst->damage_target_card)
       && (ench = in_play(inst->damage_target_player, inst->damage_target_card)))
          ...
          && (ench = in_play(affected_card_controller, affected_card))))
    {
      ...
      int setting_pt = ((new_typ & TYPE_CREATURE) && !(inst->targets[5].player == -1 && inst->targets[5].card == -1));
      if (cur_typ == new_typ // already has the right types
          ...
          && !inst->targets[4].card // not granting sp_keywords
          && (!setting_pt // either not setting power or toughness,
  ...
  if (inst->damage_target_card != -1 && in_play(inst->damage_target_player, inst->damage_target_card) && inst->targets[4].card)
    special_abilities(inst->damage_target_player, inst->damage_target_card, event, inst->targets[4].card, player, card);

  if (event == EVENT_SET_COLOR && inst->damage_target_card != -1 && affect_me(inst->damage_target_player, inst->damage_target_card)
...
{
  if (!hct_valid_card_index(tgt_player, tgt_card))
    return -1;
  ...
  if (!hct_valid_card_index(tgt_player, tgt_card) || !in_play(tgt_player, tgt_card))
    return -1;
