...
| --- | --- | --- |
| `Shandalar.exe` | Shandalar adventure shell and duel path. | Same SHA-256 as `Program/Shandalar.exe`; patched for the start-color `CreateDIBSection` crash, default-name bypass/fallback, same-arrow adventure-map stop behavior, and CrossOver/Wine callback stability around the legacy MagSnd/WinMM paths. |
