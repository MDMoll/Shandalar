#!/usr/bin/env python3
"""Patch Shandalar's WinMM timer callback to avoid async sound-service calls.

The root adventure executable installs a 33 ms timeSetEvent() callback at
0x4ce8cd. That callback increments the global tick counter, then calls the
legacy platform/sound service dispatcher at 0x56d476 from the WinMM callback
thread. Under CrossOver/Wine, repeated crashes with EIP outside all loaded
modules point at a poisoned async callback/function-pointer path rather than
card logic.

This compatibility patch keeps the tick counter intact and only NOPs the
callback-thread call to 0x56d476. The same service routine can still run from
the main window procedure path.
"""
...
CALL_VA = 0x004CE93F
PREIMAGE = bytes.fromhex("e832eb0900")
...
def patch_file(path: Path, apply: bool, backup_suffix: str | None) -> None:
    ...
    label = f"{path} WinMM tick callback call @ VA 0x{CALL_VA:x}, file 0x{offset:x}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--backup-suffix",
        default=None,
        help="preserve a sibling backup before writing, e.g. .before-winmm-tick-callback-patch",
    )
    parser.add_argument(
        "paths",
        nargs="*",
