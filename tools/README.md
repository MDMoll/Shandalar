...
| `patch-ai-etb-player-target-preselect.py` | Guarded binary patch helper for root and `Program/` `ManalinkEh.dll`; wraps the Piranha Marsh and Bojuka Bog `comes_into_play()` calls so the exact non-speculating AI land ETB effect resolves immediately during `EVENT_TRIGGER`, before Spell Chain priority. |
