...
| `patch-ai-etb-player-target-preselect.py` | Guarded binary patch helper for root and `Program/` `ManalinkEh.dll`; wraps the Piranha Marsh and Bojuka Bog `comes_into_play()` calls so exact non-speculating AI land ETB triggers store their target during `EVENT_TRIGGER`, suppress Spell Chain, and resolve during `EVENT_END_TRIGGER`. |
