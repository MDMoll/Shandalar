...
| `patch-ai-player-target-selection.py` | Guarded binary patch helper for root and `Program/` `ManalinkEh.dll` plus `Shandalar.dll`; skips the generic selector for AI-controlled player-only targets in ManalinkEh and for Shandalar AI pure player or pure in-play targets after candidate construction has already validated legal targets. |
...
| `patch-shandalar-generic-autotap.py` | Guarded binary patch helper for root and `Program/` `Shandalar.dll`; keeps the original basic-first autotap pass except for pure generic costs of 2+, where it lets two-mana nonbasic lands satisfy the cost before a one-mana basic is consumed. |
...
| `verify-loam-larva-basic-land-picker.sh` | Verifies the static Loam Larva basic-land picker fix by checking Shandalar.dll hook/cave bytes plus the source/data assumptions behind the old basic-land family handling; it does not launch the game. |
