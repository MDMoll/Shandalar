...
| Tool | Purpose |
| --- | --- |
| `verify-share-readiness.sh` | Runs automated checks for clean-tree status, ignored local clutter, expected tracked ignored files, Git binary attributes, patched runtime hashes, representative patch bytes, tracked save/local-state inventory, core docs, maintained-text ASCII, and local Markdown links. |
