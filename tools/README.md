...
| `patch-ai-etb-player-target-preselect.py` | Guarded binary patch helper for root and `Program/` `ManalinkEh.dll`; wraps the Piranha Marsh and Bojuka Bog `comes_into_play()` calls so they use the engine-native `comes_into_play_mode(..., RESOLVE_TRIGGER_AI(player))` path while their resolving blocks keep `pick_player_duh()` target selection. |
