...
check_marker "functions/targets.c" "static void append_target_error(char* error_str, size_t error_str_size, const char* err_str)" "bounded-target-error-helper"
check_marker "functions/targets.c" "append_target_error(error_str, sizeof(error_str), err_str);" "bounded-target-fail-str"
check_marker "functions/targets.c" "scnprintf(targets_global_all_purpose_buffer_ptr(), TARGETS_GLOBAL_ALL_PURPOSE_BUFFER_SIZE" "bounded-target-prepended-prompt"
...
check_marker "functions/targets.c" "static int target_player_is_valid(int player)" "bounded-target-player-helper"
check_marker "functions/targets.c" "static int target_card_slot_is_valid(int card)" "bounded-target-card-slot-helper"
...
check_marker "functions/targets.c" "bounded_targets_number_of_targets" "bounded-target-count-helper"
check_marker "functions/targets.c" "target_number < 0 || target_number >= num_targets" "bounded-validate-target-index"
...
check_marker "cards/zendikar.c" "pick_player_duh(player, card, 1-player, 0)" "piranha-marsh-ai-target"
...
check_marker "functions/ai.c" "static int ai_player_is_valid(int player)" "bounded-ai-player-helper"
...
check_marker "functions/events.c" "trigger_condition != TRIGGER_COMES_INTO_PLAY" "ai-land-cip-trigger-context"
check_marker "functions/events.c" "dispatch_event_with_attacker_to_one_card(player, card, EVENT_RESOLVE_TRIGGER, 1-player, -1);" "ai-land-cip-direct-resolve"
check_marker "functions/events.c" "instance->state &= ~(STATE_PROCESSING | STATE_IS_TRIGGERING);" "ai-land-cip-processing-clear"
check_marker "functions/events.c" "resolve_ai_land_cip_trigger_without_stack(player, card);" "ai-land-cip-resolver-guard"
check_marker "functions/damage_effects.c" "static int bounded_damage_active_cards_count(int player)" "bounded-damage-active-count-helper"
check_marker "functions/damage_effects.c" "int active_count = bounded_damage_active_cards_count(player);" "bounded-after-damage-player-scan"
...
check_marker "functions/engine.c" "ENGINE_CARD_INSTANCE_CAPACITY = 151" "bounded-engine-card-capacity"
check_marker "functions/engine.c" "static int engine_player_is_valid(int player)" "bounded-engine-player-helper"
...
check_marker "functions/sacrifices.c" "select_target(player, card, &td, prompt, &picked);" "local-sacrifice-target-buffer"
...
check_marker "drawcardlib/drawcardlib.c" "input.SuppressExternalCodecs = 1;" "drawcardlib-suppress-external-codecs"
...
check_marker "drawcardlib/drawcardlib.c" "if (stat != Ok)" "drawcardlib-gdiplus-notification-hook-status"
check_marker "drawcardlib/drawcardlib.c" "gdip_create_graphics(HDC hdc" "drawcardlib-checked-graphics-wrapper"
