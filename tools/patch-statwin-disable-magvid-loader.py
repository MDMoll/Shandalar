#!/usr/bin/env python3
"""Patch Statwin's MagVid loader to report video unavailable.

After the Shandalar MagSnd initializer was disabled, the Augur of Skulls
announcement freeze still reproduced with the same Wine debugger fault pattern:
EIP == EDX == 0xfff50de4, outside the loaded game modules. Live inspection
confirmed MagSnd.dll was no longer loaded, but Statwin.dll had loaded
magvid.dll, whose video path creates a worker thread and stores callback
function pointers for the announcement/video overlay stack.

This compatibility patch changes Statwin.dll's dynamic MagVid loader at
0x10003610 to return the wrapper family's existing "video unavailable" status
(7) immediately. Later Statwin video wrappers already short-circuit on that
state, so the patch prevents magvid.dll from loading/creating worker threads
without changing card or duel logic.
"""
...
PATCHED = bytes.fromhex("b807000000c3")
...
def patch_file(path: Path, apply: bool, backup_suffix: str | None) -> None:
    ...
    label = f"{path} MagVid loader @ VA 0x{PATCH_VA:x}, file 0x{offset:x}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--backup-suffix",
        default=None,
        help="preserve a sibling backup before writing, e.g. .before-magvid-loader-disable-patch",
    )
    parser.add_argument(
        "paths",
        nargs="*",
        default=["Statwin.dll", "Program/Statwin.dll"],
        help="Statwin.dll paths to patch; defaults to root and Program copies",
    )
