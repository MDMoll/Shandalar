...
if [ "${ALLOW_IGNORED_LOCAL:-0}" != "1" ]; then
  ignored_untracked="$(git status --ignored --short --untracked-files=all | awk '/^!! / {print substr($0, 4)}' | awk '$0 !~ /^\.codex\//')"
  if [ -n "$ignored_untracked" ]; then
    printf 'FAIL: ignored local files are present; remove them or set ALLOW_IGNORED_LOCAL=1:\n' >&2
    printf '%s\n' "$ignored_untracked" >&2
    exit 1
  fi
  pass "ignored local clutter is absent, excluding autogenerated .codex files"
else
