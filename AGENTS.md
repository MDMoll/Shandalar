...
| Root `Magic.exe` | `/Users/mdmoll/Shandalar/Shandalar` or `C:\Shandalar` in copied CrossOver bottle `MTG` | Logged root Shandalar startup opens this copy. It differs by SHA-256 from `Program/Magic.exe`, has the declared-attacker undo patch at `0x43c303`/`0x459bc8`, and currently hashes to `5bf518d66342d79562efb1106449413ada06814a6c14818a1e3101fd470c82d1`; the local `MTG` copy was patched too, with `Magic.before-declared-attacker-undo-patch.exe` preserving the previous bytes. Root `ManalinkEh.dll` is patched for the Samite/Femeref/Kithkin damage-prevention activation freeze and currently hashes to `6a5fd8057d456d691fb87810eee8dbe1680b18d1c4c79530cbe036cb443df1eb`. |
| `Program/Shandalar.exe` | `/Users/mdmoll/Shandalar/Shandalar/Program` | Same patched SHA-256 as root `Shandalar.exe`. The previous direct `C:\Shandalar\Program\Shandalar.exe` failure in bottle `MTG` was traced to missing `Program\zlib.dll`; repo and local `MTG` copied-install copies now include byte-identical `zlib.dll` at root and `Program/` with SHA-256 `9f8729ac49e0ccea86fe3b1a9b2c3fae9986ecd09db92853e7a588dbda85bf90`. Keep root Shandalar as the primary `MTG` gameplay path until direct Program launch and gameplay are visibly proven. |
| `Program/Magic.exe` | `/Users/mdmoll/Shandalar/Shandalar/Program` | Manalink launcher path. `Program/Magic.exe` and root `Magic.exe` differ by SHA-256, so test and document the path used. It has the same declared-attacker undo hook at `0x43c303`/`0x459bc8` and currently hashes to `0fb8b87fe35c8be037ae3419a9b9cd70a27df840ae6af6c7488c2685046a74fa`; the local `MTG` copy was patched too, with `Program/Magic.before-declared-attacker-undo-patch.exe` preserving the previous bytes. `Program/ManalinkEh.dll` has the same healer patch at a different offset and currently hashes to `7fc7ad86b5a3eaaa8879c76814dc454917f2e4b58acf15530e42fdcc78da2517`. |
...
| Verify the Program zlib dependency copy | `cmp -s zlib.dll Program/zlib.dll && shasum -a 256 zlib.dll Program/zlib.dll`; both paths should hash to `9f8729ac49e0ccea86fe3b1a9b2c3fae9986ecd09db92853e7a588dbda85bf90`. |
...
| Verify the current `MTG` app-default desktop/version settings | `sed -n '790,825p;965,970p' "/Users/mdmoll/Library/Application Support/CrossOver/Bottles/MTG/user.reg"`; expect `FaceMaker.exe`, `Magic.exe`, and `Shandalar.exe` app-default `Version=win7`, desktop `Shandalar1440`, and `Shandalar1440=1440x1080` under `HKCU\Software\Wine\Explorer\Desktops`. |
