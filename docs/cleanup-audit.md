...
| `CardArtNew/Thumbs.db` | Generated local-state file inside runtime-like art folder | Windows Explorer thumbnail cache, not card art. | `file CardArtNew/Thumbs.db` reported `Composite Document File V2 Document`; SHA-256 was `d613ed811f078af12887dfb5d056373606c29d036574ee31315c427bf5f101ea`; exact-path and binary-string checks found no runtime reference. | High | Removed in the safe duplicate cleanup pass; see [cleanup-removed-files.md](cleanup-removed-files.md). |
| `docs/generated/safe-cleanup/post-cleanup-largest-duplicate-groups.tsv` | Exact duplicate generated report | Byte-identical to `docs/generated/safe-cleanup/largest-duplicate-groups.tsv`; no maintained reference outside generated evidence; disposable worktree quarantine passed. | `shasum -a 256` matched both reports at `c765aca06fa871fa2b0799a8d8283f93037af49a3a388e2a2326a29063b68b48`; see [duplicate-cleanup-verification.md](duplicate-cleanup-verification.md). | High | Removed in the verified duplicate cleanup pass; kept canonical generated report. |
| `Mods/Art/*.7z` and `Mods/Rogues/*.7z` vs `Manalink3/Mods/...` | Exact duplicate archive pairs | Targeted hash audit over `*.7z`, `*.zip`, and `*.rar` under `Mods/` and `Manalink3/Mods/` found 15 duplicate hashes covering all 30 archive files in that query. | `find Mods Manalink3/Mods ... | shasum -a 256` found pairs such as `Default Sonic 2014.7z`, `Modern Sarlack MtG.7z`, `CardFrames Sonic 2014.7z`, and `Classic Microprose Default.7z`; [install-roots.md](install-roots.md) selects top-level `Mods/` as canonical. | High | Removed duplicate archive copies from `Manalink3/Mods/`; kept top-level `Mods/` archives. |
...
| `src/deploy.bat` | Historical deployment script | Confirmed mode still has hard-coded `c:\magic2k` and `c:\mingw` paths. | `src/deploy.bat:1-71`. | Medium | Keep as build-history evidence; confirmed mode only in a prepared Windows packaging copy. |
...
| Root `Shandalar.exe` | Binary duplicate but current CrossOver `MTG` launch target | Same SHA-256 as `Program/Shandalar.exe`; current `MTG` shortcut targets root `C:\Shandalar\Shandalar.exe`. The repo and local `MTG` copied install now have matching root and `Program` `zlib.dll`, but the copied Program launch path has not been visibly retested after that layout fix. | Hash comparison plus CrossOver shortcut/log evidence. | High keep | Keep; do not dedupe or move without launch-copy testing. |
...
| `Manalink3/` | Historical/unsupported package root | Contains its own `Program/`, `Mods/`, and docs; duplicate archive copies were removed from `Manalink3/Mods/` after the install-root decision. | [install-roots.md](install-roots.md) and [package-layout-cleanup.md](package-layout-cleanup.md). | Medium | Keep remaining executable-adjacent/package files as evidence unless a future package archival/removal pass moves the whole root. |
...
| --- | --- |
| Definitely generated/recreatable | `archive/generated-local/Duel.GID`, generated/log/debug files, extensionless `MAGIC5`, and empty `Program/Savedescs`. `CardArtNew/Thumbs.db` was removed in the safe duplicate cleanup pass, and one exact duplicate generated report was removed in the verified duplicate cleanup pass. |
| Likely duplicate | Full non-git duplicate hash audit, targeted deck-family duplicate hash audit, root `Shandalar.exe`. The targeted `Mods/` vs `Manalink3/Mods/` archive duplicates were resolved by install-root policy. |
...
| `Mods/` plus `Manalink3/Mods/` archives | `find Mods Manalink3/Mods -type f \( -name '*.7z' -o -name '*.zip' -o -name '*.rar' \) -print0 | xargs -0 shasum -a 256` | Before install-root cleanup: 30 archive files, 15 duplicate hashes, 30 files covered by duplicate pairs. After cleanup: duplicate `Manalink3/Mods/` archive copies removed; top-level `Mods/` copies remain. |
