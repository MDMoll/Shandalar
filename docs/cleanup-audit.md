...
| Root `Shandalar.exe` | Binary duplicate but current CrossOver `MTG` launch target | Same SHA-256 as `Program/Shandalar.exe`; current `MTG` shortcut targets root `C:\Shandalar\Shandalar.exe`. The old direct `Program` missing-`zlib.dll` blocker is fixed, but alternate-path gameplay still needs proof. | Hash comparison plus CrossOver shortcut/log evidence and 2026-06-01 `Program/zlib.dll` sync. | High keep | Keep; do not dedupe or move without launch-copy testing. |
