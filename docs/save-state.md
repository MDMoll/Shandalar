...
This repo currently includes Shandalar save slots and related local state. They
may be useful for testing, but they also make the checkout less clean for public
sharing unless intentionally kept.
...
1. Record hashes for every `MAGIC*.(SVE|map|fce)` file.
...
```sh
git ls-files | rg '(^|/)(MAGIC[0-9a-d]\.(SVE|map|fce)|CSV/MAGIC[0-9a-d]/|Savedescs|Screennames/|FaceMostRecent\.txt)'
file MAGIC*.SVE MAGIC*.map MAGIC*.fce CSV/MAGIC*/* Screennames/* FaceMostRecent.txt Savedescs
shasum -a 256 MAGIC*.SVE MAGIC*.map MAGIC*.fce FaceMostRecent.txt Screennames/*
```
