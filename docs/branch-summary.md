...
| Automated checks | Verifiers check clean tree state, ignored local clutter, generated report/handoff ignore rules, absence of tracked ignored files, binary attributes, protected cleanup false positives, tracked fresh-install payloads, install-tree hashes/patch bytes, runtime-manifest hashes, representative patch bytes, tracked save/local-state inventory, security-scan target inventory, branch-delta inventory shape, core docs, current CrossOver quick-start guidance, ASCII maintained text, docs index coverage, local Markdown links, and final controlled-maintenance gates. | [../tools/verify-share-readiness.sh](../tools/verify-share-readiness.sh), [../tools/verify-install-tree.sh](../tools/verify-install-tree.sh), [../tools/verify-final-share-gates.sh](../tools/verify-final-share-gates.sh) |
| Handoff helpers | Read-only helpers print current share status, verify that report against live counts/statuses, print gameplay/security evidence baselines, record one manual gameplay row at a time, record scanner result rows after a real scan, dry-run cleanup copies, create a Git bundle plus checksum fallback, and create temporary binary patch artifacts for patch-only planning. | [../tools/README.md](../tools/README.md), [share-readiness.md](share-readiness.md), [manual-gameplay-verification.md](manual-gameplay-verification.md), [security-scan.md](security-scan.md), [git-handoff.md](git-handoff.md), [patch-package-plan.md](patch-package-plan.md) |
...
| ClamAV scanned the tracked security-target inventory and reported no infected files. | [security-scan.md](security-scan.md), [generated/security-scan/clamav-2026-05-31.md](generated/security-scan/clamav-2026-05-31.md), and `tools/verify-security-scan-results.sh --results security-scan-results.tsv --require-all`. |
...
| Claim | Why not |
| --- | --- |
| Fully gameplay verified. | Only the main-menu and default/first start-color-to-map smoke rows are recorded as passed; the remaining manual gameplay rows still need visible Windows/CrossOver testing. |
| Final controlled-maintenance gates passed. | `tools/verify-final-share-gates.sh` should still fail until manual gameplay evidence is complete. |
...
| Guaranteed safe binaries. | ClamAV reported no infected files for the tracked scan targets, but one scanner result is not a broad safety guarantee. |
