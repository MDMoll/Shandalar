...
| Earlier direct `MTG` launch of `C:\Shandalar\Program\Shandalar.exe` with logging | Historical failure before gameplay because `Program\zlib.dll` was missing. The 2026-06-01 zlib sync should remove this loader cascade, but direct launch/gameplay needs fresh evidence. |
| `cmp -s zlib.dll Program/zlib.dll` | Match; `Program/zlib.dll` is a byte-identical copy of root `zlib.dll`. |
| `cmp -s zlib.dll "/Users/mdmoll/Library/Application Support/CrossOver/Bottles/MTG/drive_c/Shandalar/Program/zlib.dll"` | Match; the local `MTG` copied install has the same `Program/zlib.dll` bytes. |
| `shasum -a 256 zlib.dll Program/zlib.dll "/Users/mdmoll/Library/Application Support/CrossOver/Bottles/MTG/drive_c/Shandalar/Program/zlib.dll"` | All three paths hash to `9f8729ac49e0ccea86fe3b1a9b2c3fae9986ecd09db92853e7a588dbda85bf90`. This resolves the previous missing-zlib dependency layout blocker, but does not prove full `Program/Shandalar.exe` gameplay. |
| `/usr/bin/perl -e 'alarm 25; exec @ARGV' /Applications/CrossOver.app/Contents/SharedSupport/CrossOver/bin/wine --bottle MTG --workdir "C:\Shandalar\Program" "C:\Shandalar\Program\Shandalar.exe"` | Stayed alive until the alarm (`exit=142`) with no stdout/stderr in `/tmp/shandalar-program-zlib-mtg-direct.log`. The old immediate missing-`Program\zlib.dll`/`image.dll`/`DrawCardLib.dll`/`DECKDLL.dll` loader cascade was not reproduced. This is not visible gameplay proof. |
