...
| Root `Magic.exe` | Logged root Shandalar startup opens `C:\Shandalar\Magic.exe`; root and Program `Magic.exe` differ by SHA-256. | Must be tested separately from `Program/Magic.exe`. |
...
| Root `Magic.exe` | Related strings/imports; previous inventory found different SHA-256 from Program copy. | Test separately; do not assume equivalence. |
