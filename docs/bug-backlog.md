...
| 5 | B012 | `Program/Shandalar.exe` zlib layout fixed; gameplay still needs proof | medium | high | runtime path | earlier logged failure in docs; 2026-06-01 added byte-identical `Program/zlib.dll` in repo and local `MTG` copied install; bounded direct launch stayed alive until alarm with empty log | Keep root launch primary until direct Program gameplay rows are visible | manual gameplay test from exact Program path | safe |
