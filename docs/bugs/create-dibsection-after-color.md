...
| Wrong working directory or adjacent DLL layout. | Old games often depend on nearby relative assets; current `MTG` shortcut targets root `C:\Shandalar\Shandalar.exe`; direct `C:\Shandalar\Program\Shandalar.exe` fails from missing `Program\zlib.dll`. | Root and `Program` `Shandalar.exe` bytes match, so binary identity alone does not explain behavior. | Medium | Use root `C:\Shandalar\Shandalar.exe` for the current `MTG` retest; compare `Program` only after fixing its adjacent DLL layout in a copy. |
...
| Test | Result |
| --- | --- |
| Direct logged launch of `C:\Shandalar\Program\Shandalar.exe` in bottle `MTG` | Failed before gameplay because `Program\zlib.dll` was missing, cascading into `image.dll`, `DrawCardLib.dll`, and `DECKDLL.dll` import failures. |
