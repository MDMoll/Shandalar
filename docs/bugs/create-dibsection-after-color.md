...
| Wrong working directory or adjacent DLL layout. | Old games often depend on nearby relative assets; current `MTG` shortcut targets root `C:\Shandalar\Shandalar.exe`; the earlier direct `C:\Shandalar\Program\Shandalar.exe` failure was caused by missing `Program\zlib.dll`, now fixed at the dependency-layout level. | Root and `Program` `Shandalar.exe` bytes match, so binary identity alone does not explain behavior. | Medium | Use root `C:\Shandalar\Shandalar.exe` for the current `MTG` gameplay retest; compare `Program` after recording a bounded direct-launch result with the fixed adjacent DLL layout. |
...
| Test | Result |
| --- | --- |
| Earlier direct logged launch of `C:\Shandalar\Program\Shandalar.exe` in bottle `MTG` | Historical failure before gameplay because `Program\zlib.dll` was missing, cascading into `image.dll`, `DrawCardLib.dll`, and `DECKDLL.dll` import failures. `Program/zlib.dll` is now present in repo and local `MTG`; bounded re-test is needed. |
