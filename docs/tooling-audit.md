...
| `tools/check-source-snapshot-parity.sh` | Read-only `src/` versus `Program/src/` parity checker. | Reports source divergence but does not choose the authoritative snapshot. | Records 81 matching paths, 141 differing paths, and 110 paths missing from `Program/src`; exact-match files and current safety markers pass. | Run after mirrored source-safety edits and before source-to-runtime claims. | yes |
