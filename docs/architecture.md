...
| `Program/FaceMaker.exe` and `Program/FaceMaker-nores.exe` | `Program/FaceMaker-nores.exe` still matches the known Korath/no-resolution helper. Active `Program/FaceMaker.exe` and root `FaceMaker.exe` are patched at file offset `0x5f40`, hash to `41f062874f94d732cc4feb40b568728b8462879fd3ec2bc55810f118e9c5f246`, and differ from the no-resolution reference by 11 bytes. Direct patched root FaceMaker launch in `MTG` rendered without the DIB assertion. | User testing says the original FaceMaker swap alone did not fix the start-color assertion; direct FaceMaker startup still does not prove Shandalar-spawned character creation. |
