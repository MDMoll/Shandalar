...
| --- | --- | --- |
| Root executable/assets | Current repo-root Shandalar launch surface and copied CrossOver `MTG` launch surface. | Verified by shortcut target, logged `C:\Shandalar\Shandalar.exe` startup, root DLL/assets, root `Magic.exe` access, patched repo `Shandalar.exe` start-color smoke test, and static WinMM timer-callback patch bytes. |
...
| --- | --- | --- |
| Root `Shandalar.exe` and `Program/Shandalar.exe` | Same active SHA-256 in this checkout and copied `MTG` install: `ebba01ad04aba5fb78841f37b6c264dfd17f1d6ca6ccfcc9851c2972b64f5f6b`. Both include the `CreateDIBSection hSection = NULL` compatibility patch, the `mPlayer` default-name seed, the name-editor bypass/fallback, the same-arrow movement-stop code cave, a MagSnd update-message compatibility patch at VA `0x4ce62e` / file offset `0xcda2e`, a minimal WinMM timer-callback entry at VA `0x4ce8cd` / file offset `0xcdccd`, the earlier callback-thread `call 0x56d476` NOP at VA `0x4ce93f` / file offset `0xcdd3f`, a MagSnd init-disable compatibility patch at VA `0x56cf20` / file offset `0x16c320`, and an MCIWndCreateA disable stub at VA `0x578c10` / file offset `0x178010`. Current `MTG` shortcut targets root `C:\Shandalar\Shandalar.exe`, local bottle copies were patched with backups, and this repo plus the local `MTG` copy now have `Program/zlib.dll`, adjacent Program config/font/card-data files, plus Program CardArt assets observed so far through `Modern/CardOv_Nyx.png`. The 2026-06-08 priestess pre-card freeze showed the same `EIP == EDX == 0xfff50de4` fault after the previous patches were present and after `Magic.exe`, `MagSnd.dll`, `magvid.dll`, and `ManalinkEh.dll` were absent; MSVFW32 was still loaded, motivating the MCIWndCreateA disable. | Either path may still behave differently because adjacent DLLs/assets differ; the direct copied-bottle Program path failed before the DLL layout fix and then exposed missing Program CardArt/config/font assets plus older Program card-data files. The latest bounded log opened the Program card-data trio, `shandalar.dll`, and `Shandalar.ini` without the earlier fatal strings; full character creation, movement-stop behavior, adventure-mode sound behavior, old StatWin/AVI video behavior, and broad prompt/button stability still need visible manual gameplay verification. |
| Root `Shandalar.dll` and `Program/Shandalar.dll` | Same active SHA-256 in this checkout and copied `MTG` install: `3a20ba36dabef6f5ff9be3a1990d8e959570764d4dff2ff88de0cea01d534f41`. Both include Shandalar adventure-duel hooks for the AI land CIP resolver stack-bypass at `0x94d34`/`0x1174800`, the C++ AI pure player/in-play target selector at `0xcb16`/`0x1174920`, the basic-land library-selector guard at `0x7e395`/`0x1174960`, and the pure-generic autotap order guard at `0x825c0`/`0x1174980`. | Static bytes prove the hooks are present, and Loam Larva has a user-confirmed 2026-06-11 pass. Merfolk Sovereign, Augur/Piranha/Bojuka, Jungle Basin/Copper Myr autotap, and broader prompt stability still need visible retests before claiming gameplay stability. |
| Root `Statwin.dll` and `Program/Statwin.dll` | Same active SHA-256 in this checkout and copied `MTG` install: `f1428cf548810f85df6f26b913d10dca16bc0f06a609a94c0cb0f0308347b0cf`. Both include a MagVid loader-disable patch at VA `0x10003610` / file offset `0x2a10` that returns video-unavailable before loading `magvid.dll`. | Static bytes prove the loader hook is present, and live Augur evidence showed `MagSnd.dll` was absent while `Statwin.dll`/`magvid.dll` were loaded before this patch. Fresh visible Augur/prompt retesting is still required, and old StatWin/AVI video behavior may be unavailable. |
...
| Root `ManalinkEh.dll` and `Program/ManalinkEh.dll` | Both are patched for the shared Samite/Femeref/Kithkin damage-prevention handler, the generic activated `GAA_DAMAGE_PREVENTION*` helper guard, AI decision-time clamping, AI raw-mana speculation snapshot restore safety, Myr Moonvessel dies-trigger resolution, Piranha Marsh/Bojuka Bog trigger targeting, generic AI player-only target selection, and AI land CIP resolver stack-bypass handling. Root patch sites are `0x3bb035`, `0x44cb23` with cave `0x495a30`, `0x40d0e1` with cave `0x495a60`, `0x40db84` with cave `0x495a90`, Myr Moonvessel at `0xb9d40`, `0x3fe7a0`, `0x3f63e0`, `0x469583` with cave `0x495ad0`, and resolver hook/cave `0x429acf`/`0x495b00` with restored Piranha/Bojuka callsites at `0x3fe77d`/`0x3f63bd`, SHA-256 `0a2d77aa15fd18648a99398d7bb45b97e47a1a8ea6c01dc3e22433851940a507`; `Program/` patch sites are `0x381a25`, `0x40f115` with cave `0x452c30`, `0x3d2da1` with cave `0x452c60`, `0x3d3844` with cave `0x452c90`, Myr Moonvessel at `0xb1cd0`, `0x3c4930`, `0x3bc630`, `0x429453` with cave `0x452cd0`, and resolver hook/cave `0x3ec7cf`/`0x452d00` with restored Piranha/Bojuka callsites at `0x3c490d`/`0x3bc60d`, SHA-256 `e715b92495677abf940b3bfda438477d66896532b24c1b05a4ea2bc2179c2e22`. | The DLLs differ by path and hash, so copy/test the one adjacent to the exact `Magic.exe` path being launched. The local `MTG` copied install now matches these Manalink hashes, but visible duel retesting is still required. |
