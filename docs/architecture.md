...
| Root `ManalinkEh.dll` and `Program/ManalinkEh.dll` | Both are patched for the shared Samite/Femeref/Kithkin damage-prevention handler, the generic activated `GAA_DAMAGE_PREVENTION*` helper guard, AI decision-time clamping, AI raw-mana speculation snapshot restore safety, Piranha Marsh/Bojuka Bog trigger targeting, generic AI player-only target selection, and AI ETB player-target end-trigger resolution. Root patch sites are `0x3bb035`, `0x44cb23` with cave `0x495a30`, `0x40d0e1` with cave `0x495a60`, `0x40db84` with cave `0x495a90`, `0x3fe7a0`, `0x3f63e0`, `0x469583` with cave `0x495ad0`, and `0x3fe77d`/`0x3f63bd` with cave `0x495b00`, SHA-256 `7cb25032ee48c973b6e5ce17195607b5e3472ea457d60cc9421c320becadd927`; `Program/` patch sites are `0x381a25`, `0x40f115` with cave `0x452c30`, `0x3d2da1` with cave `0x452c60`, `0x3d3844` with cave `0x452c90`, `0x3c4930`, `0x3bc630`, `0x429453` with cave `0x452cd0`, and `0x3c490d`/`0x3bc60d` with cave `0x452d00`, SHA-256 `f0a399ae7a8d65144f0d9bafff5a9287140c7c056fa0468d1cd9b5b07f3404d4`. | The DLLs differ by path and hash, so copy/test the one adjacent to the exact `Magic.exe` path being launched. The local `MTG` copied install now matches these Manalink hashes, but visible duel retesting is still required. |
