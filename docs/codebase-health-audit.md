...
| Bounded engine phase and mana prompt formatting. | `src/functions/engine.c`; `Program/src/functions/engine.c`; `tools/check-source-snapshot-parity.sh` | Source-only formatting hardening replaces phase trace `sprintf()` calls and local mana-cost fragment appends with bounded `scnprintf()`/`vscnprintf()` helpers; no shipped DLL changed. | Static grep/review of remaining `engine.c` `sprintf()` calls; source parity marker checks; source dry-runs |
| Bounded deck/exile/tutor diagnostics and prompts. | `src/functions/deck.c`; `Program/src/functions/deck.c`; `src/functions/exiledby.c`; `Program/src/functions/exiledby.c`; `src/functions/tutors.c`; `Program/src/functions/tutors.c`; `tools/check-source-snapshot-parity.sh` | Source-only formatting hardening replaces small-buffer diagnostic, exile-title, draw-loss, and graveyard-selection `sprintf()`/`strcpy()` calls with bounded `scnprintf()` calls; no shipped DLL changed. | Static grep/review of targeted helpers; source parity marker checks; source dry-runs |
...
| Guarded Shandalar deckbuilder minimum deck size. | `src/deck/deckdll.cpp`; `DeckDLL.dll`; `Program/Deckdll.dll`; `tools/inspect-shandalar-save-decks.py`; `tools/verify-crossover-mtg-state.sh`; `docs/bugs/summoned-wizard-deck-handoff.md` | DeckDLL guard blocks closing/switching away from player deck views while deck 1 is below 40 or any nonempty alternate deck is below 40; rebuilt root/Program DLLs now carry the guard at SHA-256 `a9cea247d80fe457e72d94055bd3b1e8a191ce9d3389b9fd5d4d42f40cb1e0d8`, and local save inspection can check all nonempty save decks. | `make` in `src/deck` with MinGW overrides; repo and `MTG` bottle hash parity; save-deck inspector checks against repo and `MTG` saves; source/build dry-runs |
