...
| B025 | Bojuka Bog used the same generic player-target path during an opponent-controlled ETB trigger. | runtime mitigated | Source snapshots, current repo DLLs, and local `MTG` copied-install DLLs now route the trigger through `pick_player_duh()`; visible Bojuka Bog retesting is still required. |
| B026 | Mandatory AI-controlled player-only ETB triggers could fall through to the generic selector. | runtime mitigated | Source snapshots, current repo DLLs, and local `MTG` copied-install DLLs now short-circuit AI `TARGET_ZONE_PLAYERS` targets after candidate construction; broad ETB retesting is still required. |
...
| Patched Bojuka Bog trigger target selection. | `src/cards/worldwake.c`; `Program/src/cards/worldwake.c`; `ManalinkEh.dll`; `Program/ManalinkEh.dll`; `tools/patch-bojuka-bog-trigger-target.py`; `tools/check-source-snapshot-parity.sh`; `docs/bugs/ai-etb-player-target-spell-chain-freeze.md` | Source, repo-runtime, and local `MTG` copied-install paths now use `pick_player_duh()` for the same mandatory player-target ETB trigger pattern while preserving normal human choice. | `python3 tools/patch-bojuka-bog-trigger-target.py`; `tools/check-source-snapshot-parity.sh`; `tools/verify-crossover-mtg-state.sh`; `objdump` site checks; `ALLOW_DIRTY=1 tools/verify-share-readiness.sh` |
| Patched generic AI player-only target selection. | `src/functions/targets.c`; `Program/src/functions/targets.c`; `ManalinkEh.dll`; `Program/ManalinkEh.dll`; `tools/patch-ai-player-target-selection.py`; `tools/check-source-snapshot-parity.sh`; `docs/bugs/ai-etb-player-target-spell-chain-freeze.md` | Source, repo-runtime, and local `MTG` copied-install paths now bypass `sub_499050()` for AI non-speculating pure player targets after candidate construction has already applied `preferred_controller`. | `python3 tools/patch-ai-player-target-selection.py`; `tools/check-source-snapshot-parity.sh`; `tools/verify-crossover-mtg-state.sh`; `objdump` cave checks; `ALLOW_DIRTY=1 tools/verify-share-readiness.sh` |
