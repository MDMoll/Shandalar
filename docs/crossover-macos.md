...
| 11 | For duel-freeze retests, use the patched `ManalinkEh.dll`, `Shandalar.dll`, `Shandalar.exe`, and `Statwin.dll` files too. The local `MTG` copied install has already been updated with backups for the Femeref/Samite/Kithkin healer guard, generic activated damage-prevention guard, AI decision-time clamp, AI raw-mana snapshot patch, Piranha Marsh trigger-target patch, Bojuka Bog trigger-target patch, Manalink/Shandalar generic AI target-selector patch, AI land CIP resolver stack-bypass handling patch, Shandalar MagSnd callback/init mitigations, and Statwin MagVid loader-disable mitigation. Repeat the backup/copy pattern for any other copied CrossOver install only with explicit approval before retesting there. |
...
| --- | --- | --- |
| Shandalar in copied `MTG` bottle | `C:\Shandalar\Shandalar.exe` | `C:\Shandalar` | Bottle-local copy is now patched; config-only fixes did not solve user retesting before these binary mitigations. Root and Program `Statwin.dll` are also patched to keep `magvid.dll` from loading. |
| Root Magic in copied `MTG` bottle | `C:\Shandalar\Magic.exe` | `C:\Shandalar` | Opened by root `Shandalar.exe`; a bounded `--e 0442 --p 0442` direct-duel log also opened it without targeted coin-flip/fatal strings. Root `ManalinkEh.dll` is patched for damage-prevention, AI clamp, AI raw-mana snapshot, Piranha Marsh/Bojuka Bog trigger-target retests, generic AI player-only target selection, and AI land CIP resolver stack-bypass handling. |
| Program Magic in copied `MTG` bottle | `C:\Shandalar\Program\Magic.exe` | `C:\Shandalar\Program` | Manalink launcher path; `Program\ManalinkEh.dll` is patched for equivalent damage-prevention, AI clamp, AI raw-mana snapshot, Piranha Marsh/Bojuka Bog trigger-target testing, generic AI player-only target selection, and AI land CIP resolver stack-bypass handling. |
...
Later follow-ups added adventure-map movement, WinMM timer-callback, and MagSnd
update-message compatibility patches. The movement hooks at `0x44398c`,
`0x444a2b`, and `0x444aa7` use code cave `0x46502d` and flag `0x583a2c` so a
repeat press of the active movement key can route through the existing stop path
at `0x444a96`; the MagSnd patch NOPs the shell/window private `UpdateSnd`
message call at file offset `0xcda2e`; the WinMM patch NOPs the callback-thread
`call 0x56d476` at file offset `0xcdd3f`.
...
This checks the copied `C:\Shandalar` runtime hashes, representative Magic,
Shandalar, Statwin, and Manalink patch bytes, FaceMaker support files, `Window = 2`,
...
| 2026-05-30 | 26.1.0.39808 | `MTG` and `Shandalar-Win8-Test` | Bottle copies | Patched copied `Shandalar.exe` files | Completed | None | Original copies backed up as `Shandalar.before-hsection-null-patch.exe` | Four bottle-local `Shandalar.exe` copies hashed to the combined-patch value `ad9ee80e0d377e7f1741e48aa0e33c3a8d7bd2873d43045e32bc42812aaa284b` at this point; the older hSection-only value was `73aa1400ddc452462f4e714e349ff06d4564c133408cf2ab10e576ae65d441b9`. |
| 2026-05-30 | 26.1.0.39808 | `MTG` and `Shandalar-Win8-Test` | Bottle copies | Copied movement-stop patched `Shandalar.exe` files | Completed | None | Same active binaries as repo root and `Program/` at that point | Four bottle-local `Shandalar.exe` copies hashed to `ad9ee80e0d377e7f1741e48aa0e33c3a8d7bd2873d43045e32bc42812aaa284b` at this point; same-arrow stop still needs visible gameplay testing. |
| 2026-05-30 | 26.1.0.39808 | `MTG` and `Shandalar-Win8-Test` | Bottle copies | Copied name-editor bypass/fallback patched `Shandalar.exe` files | Completed | None | Same active binaries as repo root and `Program/` at that point | Four bottle-local `Shandalar.exe` copies hashed to `ad9ee80e0d377e7f1741e48aa0e33c3a8d7bd2873d43045e32bc42812aaa284b` at this point; this row was superseded by the later 2026-05-31 visible S2 run that reached the map in `MTG`. |
...
| 2026-06-11 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `Shandalar.dll` and `Program/Shandalar.dll` | Broadened Shandalar C++ AI target selector | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | Manual Merfolk Sovereign testing froze at the `Merfolk Shaman activates...` announcement after the Loam Larva retest passed. The hook at `0xcb16` now jumps to cave `0x1174920` and chooses candidate `0` for non-speculating AI pure player or pure in-play targets; fresh visible Merfolk Sovereign/Augur/Piranha/Bojuka retests still needed after restart. |
...
| 2026-06-07 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `Shandalar.exe` and `Program/Shandalar.exe` | Patched MagSnd update-message service call | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | Manual Glowing Anemone testing froze at the human `Trigger` / `Decline` prompt after target selection, broadening the issue beyond AI targeting. Static analysis found the remaining shell/window private `0x10101010` MagSnd `UpdateSnd` message call into the same legacy wrapper at `0x56d476`; the patch NOPs that call at VA `0x4ce62e` / file offset `0xcda2e`. Fresh visible prompt/button stability retest is required. |
| 2026-06-07 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `Shandalar.exe` and `Program/Shandalar.exe` | Disabled adventure-mode MagSnd initialization | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | Manual Thoughtseize testing froze with the `Spell Chain` pane movable and its card render corrupted, while the cast announcement overlay rendered the same card cleanly. Wine Debugger again showed `EIP == EDX == 0xfff50de4`, and live inspection showed `MagSnd.dll` loaded despite the previous two call-site patches. The new patch makes the initializer wrapper at VA `0x56cf20` / file offset `0x16c320` return sound-unavailable before loading `MagSnd.dll`; fresh visible prompt/button stability and sound-behavior retesting are required. |
| 2026-06-07 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `Statwin.dll` and `Program/Statwin.dll` | Disabled dynamic MagVid loader | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | Manual Augur of Skulls testing still froze at `Witch activates...` after the MagSnd init patch. Live inspection confirmed `MagSnd.dll` was absent, while `Statwin.dll` and `magvid.dll` were loaded; Wine Debugger again showed `EIP == EDX == 0xfff50de4`. The new patch makes the Statwin loader at VA `0x10003610` / file offset `0x2a10` return video-unavailable before `magvid.dll` can load or create worker threads. Fresh visible Augur/prompt retesting is required, and old StatWin/AVI video behavior may be unavailable. |
| 2026-06-07 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `Shandalar.exe` and `Program/Shandalar.exe` | Reduced WinMM timer callback body | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | Manual priestess testing froze before cards appeared. Live inspection confirmed `MagSnd.dll` and `magvid.dll` were absent, but Wine Debugger again showed `EIP == EDX == 0xfff50de4` in a worker thread. The new patch changes the `timeSetEvent()` callback entry at VA `0x4ce8cd` / file offset `0xcdccd` to only increment `0x589df0` and return with `ret 0x14`, preserving timer waits while avoiding the callback's thread-handle and priority bookkeeping. Fresh visible duel-start and prompt/button stability retesting is required. |
