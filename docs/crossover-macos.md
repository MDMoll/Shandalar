...
| 11 | For duel-freeze retests, use the patched `ManalinkEh.dll` too. The local `MTG` copied install has already been updated with backups for the Femeref/Samite/Kithkin healer guard, generic activated damage-prevention guard, AI decision-time clamp, AI raw-mana snapshot patch, Piranha Marsh trigger-target patch, and Bojuka Bog trigger-target patch. Repeat the backup/copy pattern for any other copied CrossOver install only with explicit approval before retesting there. |
