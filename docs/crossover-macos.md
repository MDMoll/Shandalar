...
| 2026-06-06 | 26.1.0.39808 | Repo and `MTG` | App-default `win7` | `ManalinkEh.dll` and `Program/ManalinkEh.dll` | Patched AI ETB player-target end-trigger resolution for Piranha Marsh and Bojuka Bog | None from static verification | Guarded patch helper plus byte/hash checks; `tools/verify-crossover-mtg-state.sh` | `xxd` and `objdump` verify the wrapper stores the exact non-speculating AI target during `EVENT_TRIGGER`, suppresses Spell Chain, and resolves during `EVENT_END_TRIGGER`; fresh visible Bojuka/Piranha retest still needed. |
