...
| `shasum -a 256 ManalinkEh.dll Program/ManalinkEh.dll` | repo root | Root patched hash is `752b6deb941cf75dfc846c023b78000766b7c79b7ef7c35505f5de830f08fd22`; `Program/` patched hash is `5dc6724b13b0ac3817561d4cad3e30a6f67fb6a2d45f2e259c715c46f08bcb9d`. |
...
| `xxd -p` checks at `0x3f63bd`/`0x3fe77d`/`0x495b00` for root `ManalinkEh.dll` and `0x3bc60d`/`0x3c490d`/`0x452d00` for `Program/ManalinkEh.dll` | repo root | Piranha Marsh and Bojuka Bog use `comes_into_play_mode(..., RESOLVE_TRIGGER_AI(player))` for trigger discovery while their resolving blocks keep `pick_player_duh()`. Fresh manual Bojuka proof is still pending after the previous end-trigger candidate failed. |
