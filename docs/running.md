...
| --- | --- | --- | --- | --- | --- |
| `Shandalar.exe` | Shandalar adventure shell and duel entry path. | repo root or `C:\Shandalar` in copied bottle | Root DLLs, `Shandalar.ini`, `Cards.dat`, `DBInfo.dat`, `Rarity.dat`, art/sound/resource folders, root `Magic.exe`, patched root `Statwin.dll`. | Present, PE32 i386, and patched for the start-color `CreateDIBSection` crash, default-name seed/bypass/fallback, same-arrow movement stop, minimal WinMM timer-callback compatibility, MagSnd update-message compatibility, MagSnd initialization-disable behavior, and MCIWndCreateA disable behavior. The adjacent `Statwin.dll` is patched to return video-unavailable before loading `magvid.dll`. In `Shandalar-Win8-Test`, the hSection patch passed the reported crash point; the broader prompt/button stability patches still need visible manual verification. | Same SHA-256 as `Program/Shandalar.exe`; local copied bottle installs were patched with backups. |
...
| `shasum -a 256 Shandalar.exe Program/Shandalar.exe` | repo root | Match; root and `Program/` Shandalar binaries hash to `ebba01ad04aba5fb78841f37b6c264dfd17f1d6ca6ccfcc9851c2972b64f5f6b`. |
...
| `xxd -p -l 16 -s $((0xcdccd)) Shandalar.exe` and `Program/Shandalar.exe` | repo root | Both patched dumps are `ff05f09d5800c2140090909090909090`, reducing the WinMM timer callback to the required tick increment and callback return. |
| `xxd -p -l 5 -s $((0xcdd3f)) Shandalar.exe` and `Program/Shandalar.exe` | repo root | Both patched dumps are `9090909090`, preserving the earlier NOP of the callback-thread `call 0x56d476`. |
| `xxd -p -l 5 -s $((0xcda2e)) Shandalar.exe` and `Program/Shandalar.exe` | repo root | Both patched dumps are `9090909090`, NOPing the shell/window private MagSnd `UpdateSnd` message call after `0x10101010`. |
| `xxd -p -l 6 -s $((0x178010)) Shandalar.exe` and `Program/Shandalar.exe` | repo root | Both patched dumps are `31c0c3909090`, making Shandalar's direct `MCIWndCreateA` thunk return `NULL` before entering Wine's MSVFW32 MCI window/video path. |
| `xxd -p -l 6 -s $((0x2a10)) Statwin.dll` and `Program/Statwin.dll` | repo root | Both patched dumps are `b807000000c3`, making the Statwin dynamic MagVid loader return video-unavailable before loading `magvid.dll`; both files hash to `f1428cf548810f85df6f26b913d10dca16bc0f06a609a94c0cb0f0308347b0cf`. |
...
| `shasum -a 256 ManalinkEh.dll Program/ManalinkEh.dll` | repo root | Root patched hash is `0a2d77aa15fd18648a99398d7bb45b97e47a1a8ea6c01dc3e22433851940a507`; `Program/` patched hash is `e715b92495677abf940b3bfda438477d66896532b24c1b05a4ea2bc2179c2e22`. |
...
| CrossOver `MTG` bottle | patched root `C:\Shandalar\Statwin.dll` with root `C:\Shandalar\Shandalar.exe` | Prompt/announcement paths should no longer load `magvid.dll` through Statwin's dynamic loader. | Repo and copied `MTG` root/Program `Statwin.dll` copies are patched and hash-checked; visible Augur/prompt retest still needed. |
