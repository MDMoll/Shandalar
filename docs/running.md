...
| `Magic.exe` | Duel executable opened by root `Shandalar.exe`. | repo root or `C:\Shandalar` in copied bottle | Root Manalink DLLs, card/art/deck/sound folders. | Present and PE32 i386. Opened by logged root Shandalar startup. | Different SHA-256 from `Program/Magic.exe`; test separately. |
...
| `Program/Magic.exe` | Manalink / launcher duel executable. | `Program/` | `ManalinkEh.dll`, `ManalinkEx.dll`, `Deckdll.dll`, `Drawcardlib.dll`, `CardArtLib.dll`, art/deck/sound folders. | Present and PE32 i386. Earlier CrossOver run from `Program/` exited 53 with no visible result. | Treat as a first-class target. |
...
| Virtual desktop | `Shandalar`, `1024x768` |
| App-default Windows version | `win8` for `Shandalar.exe`, `Magic.exe`, and `FaceMaker.exe` |
