...
| Security scan evidence helpers | Scanner testing still has to be done with a real scanner, but local scanner availability can be checked and current target rows can be generated, recorded, and validated against exact tracked hashes. | [security-scan.md](security-scan.md), `tools/check-security-scanner-availability.sh`, `tools/create-security-scan-results-template.sh`, `tools/record-security-scan-result.sh`, `tools/verify-security-scan-results.sh`. |
