...
| Automated non-gameplay checks | Repo share-readiness checks are repeatable without launching the game, including ignored-local-clutter checks, tracked save/local-state inventory, maintained-text ASCII, and local Markdown link validation. | `tools/verify-share-readiness.sh`. |
