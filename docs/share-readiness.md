...
| Final gate command | A strict controlled-maintenance verifier exists, but it is expected to fail until the branch is pushed, manual gameplay evidence is complete, and scanner-result coverage is full. | `tools/verify-final-share-gates.sh`. |
