...
| Current share status | A single read-only report prints the active branch, commit, inventory counts, expected handoff artifact paths, artifact hashes when present, and remaining gates without claiming gameplay or scan proof. | `tools/print-share-status.sh`. |
