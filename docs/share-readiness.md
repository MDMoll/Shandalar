...
| Git handoff | Branch, remote, pre-push checks, known auth failure, owner-side auth paths, Git bundle/patch fallback, checksum sidecars, and artifact verification are recorded. | [git-handoff.md](git-handoff.md), [push-auth.md](push-auth.md), `tools/create-git-handoff-bundle.sh`, `tools/create-patch-package.sh`, `tools/verify-handoff-artifacts.sh`. |
...
| Current share status | A single read-only report prints the active branch, commit, inventory counts, expected handoff artifact paths, artifact hashes when present, local scanner-result validation status, and remaining gates without claiming gameplay or scan proof. | `tools/print-share-status.sh`. |
| Automated non-gameplay checks | Repo share-readiness and handoff checks are repeatable without launching the game, including ignored-local-clutter checks, generated report/handoff ignore checks, protected cleanup false positives, runtime-manifest hash checks, tracked save/local-state inventory, security-scan target inventory, optional scanner-result TSV validation, branch-delta inventory shape, maintained-text ASCII, docs index coverage, local Markdown link validation, evidence-baseline sanity checks, share-status artifact hash checks, cleanup-copy dry-runs for default no-`.git` and `--include-git` modes, bundle/patch dry-runs, optional bundle-import verification, and optional default artifact verification. | `tools/verify-share-readiness.sh`, `tools/verify-handoff-readiness.sh`, `tools/verify-security-scan-results.sh`. |
...
remote and recorded failure, and [push-auth.md](push-auth.md) for HTTPS token,
GitHub CLI, and SSH options.
