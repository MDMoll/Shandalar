...
| --- | --- | --- | --- | --- | --- |
| `src/` | C, C++, ASM, Perl, make | Main source snapshot and binary patch scripts. | runtime-adjacent; not proven to rebuild shipped files | dry-run reaches build plan | `src/card_id.h` was restored from `Program/src/card_id.h`; real builds still need missing Windows DLL tools and source provenance proof. |
