...
| Retest a duel where the opponent plays Piranha Marsh. | The runtime now routes that ETB trigger through `pick_player_duh()` and immediately resolves the exact non-speculating AI land ETB effect before Spell Chain priority. |
| Retest a duel where the opponent plays Bojuka Bog. | Manual gameplay showed the selector-side and preselection-only mitigations were not enough; the runtime now immediately resolves the exact non-speculating AI land ETB effect before Spell Chain priority. |
