...
| Name-entry and callback binary patches applied to `Shandalar.exe` and `Program/Shandalar.exe` | File offset `0xa1a42` writes `mPlayer` into the buffer at `0x591228`; `0xa1acd` bypasses the fragile manual name editor by setting its return value to success; `0xa1af2` jumps to code at `0x465170`/file offset `0x64570` that writes `Player` if the accepted buffer is empty before copying to `0x7a0770`; `0xcda2e` NOPs the shell/window private MagSnd `UpdateSnd` message call; `0xcdd3f` NOPs the WinMM callback-thread call to `0x56d476`; `0x16c320` returns `b8 04 00 00 00 c3` from the MagSnd initializer before `MagSnd.dll` is loaded. The name-entry-only interim hash was `bd784cc248d08455270a6bfae5004ead8f9723d8017f8db152add113e8d3a9db`; the prior seed-plus-movement hash was `155a668c72867bd1274410eb05ca05fbb7bd9bed843b42d1583ea536805a4aaf`; the later DIB/name/movement hash was `ad9ee80e0d377e7f1741e48aa0e33c3a8d7bd2873d43045e32bc42812aaa284b`; both active files now hash to `c0d3607d991c976a2302880fb3cb341c096b570935bf0f27b91a732b45119a2b` after the callback and MagSnd-init patches. |
...
| Bottle copies patched | `MTG` copied `Shandalar.exe` files now hash to `c0d3607d991c976a2302880fb3cb341c096b570935bf0f27b91a732b45119a2b`; originals were backed up as `Shandalar.before-hsection-null-patch.exe` before the first DIB patch, with later `Shandalar.before-winmm-tick-callback-patch.exe` backups before the WinMM timer-callback patch, `Shandalar.before-magsnd-update-patch.exe` backups before the MagSnd update-message patch, and `Shandalar.before-magsnd-init-disable-patch.exe` backups before disabling adventure-mode MagSnd initialization. The older `Shandalar-Win8-Test` comparison bottle was not present during the 2026-06-07 MagSnd verification. |
