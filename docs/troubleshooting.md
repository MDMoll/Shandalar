...
| Retest a duel where the opponent plays Piranha Marsh. | The runtime now routes that ETB trigger through `comes_into_play_mode(..., RESOLVE_TRIGGER_AI(player))` and keeps `pick_player_duh()` for the resolving target. |
| Retest a duel where the opponent plays Bojuka Bog. | Manual gameplay showed the selector-side, preselection-only, trigger-time immediate-resolution, and end-trigger suppression mitigations were not enough; the runtime now uses engine-native AI trigger mode with `pick_player_duh()` for the resolving target. |
