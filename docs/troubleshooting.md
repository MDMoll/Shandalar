...
| Retest a duel where the opponent plays Piranha Marsh. | The runtime now routes that ETB trigger through `pick_player_duh()`, stores the exact non-speculating AI target during `EVENT_TRIGGER`, suppresses Spell Chain, and resolves during `EVENT_END_TRIGGER`. |
| Retest a duel where the opponent plays Bojuka Bog. | Manual gameplay showed the selector-side, preselection-only, and trigger-time immediate-resolution mitigations were not enough; the runtime now stores the exact non-speculating AI target during `EVENT_TRIGGER`, suppresses Spell Chain, and resolves during `EVENT_END_TRIGGER`. |
