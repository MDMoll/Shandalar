...
| Bottle copies patched | `MTG` copied `Shandalar.exe` files now hash to `17f7af843fd2fd5424e7d36d547f4315d20fdfa840fb5050a96ab9a727a181f6`; originals were backed up as `Shandalar.before-hsection-null-patch.exe` before the first DIB patch, with later `Shandalar.before-winmm-tick-callback-patch.exe` backups before the first WinMM timer-callback patch, `Shandalar.before-magsnd-update-patch.exe` backups before the MagSnd update-message patch, `Shandalar.before-magsnd-init-disable-patch.exe` backups before disabling adventure-mode MagSnd initialization, and `Shandalar.before-minimal-winmm-timer-callback-patch.exe` backups before reducing the WinMM callback body. The older `Shandalar-Win8-Test` comparison bottle was not present during the 2026-06-07 MagSnd verification. |
