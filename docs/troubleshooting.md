...
| Retest a duel where the opponent plays Piranha Marsh. | The runtime now routes that ETB trigger through `pick_player_duh()` and preselects the AI player target before Spell Chain priority. |
| Retest a duel where the opponent plays Bojuka Bog. | Manual gameplay showed the selector-side mitigation was not enough; the runtime now also preselects the AI player target before Spell Chain priority. |
