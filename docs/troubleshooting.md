...
| Bottle copies patched | `MTG` copied `Shandalar.exe` files now hash to `92cca05b493c28f6c29c0cc4bd0018499acd9a8cbdce06f9230da59d5be0a0ef`; originals were backed up as `Shandalar.before-hsection-null-patch.exe` before the first DIB patch, with later `Shandalar.before-winmm-tick-callback-patch.exe` backups before the WinMM timer-callback patch. The older `Shandalar-Win8-Test` comparison bottle was not present during the 2026-06-07 WinMM verification. |
