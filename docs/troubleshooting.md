...
| Bottle copies patched | `MTG` copied `Shandalar.exe` files now hash to `bec1dd2bba524618529d674e3a927f4aa93f53a39a444dd3d6d425856f8c1b32`; originals were backed up as `Shandalar.before-hsection-null-patch.exe` before the first DIB patch, with later `Shandalar.before-winmm-tick-callback-patch.exe` backups before the WinMM timer-callback patch and `Shandalar.before-magsnd-update-patch.exe` backups before the MagSnd update-message patch. The older `Shandalar-Win8-Test` comparison bottle was not present during the 2026-06-07 MagSnd verification. |
