...
| Test | Why |
| --- | --- |
| Fully quit existing CrossOver Shandalar/Magic windows before retesting root `C:\Shandalar\Shandalar.exe`. | The local `MTG` copied install now has matching damage-prevention, AI-timer, raw-mana snapshot, Piranha Marsh trigger-target, Bojuka Bog trigger-target, generic AI target-selector, and AI land CIP resolver stack-bypass patched `ManalinkEh.dll` files, plus matching patched `Shandalar.dll` files for the adventure-duel land resolver, pure player/in-play targeter, and Loam Larva subtype paths. Already-running processes can still hold older DLLs. |
...
| Retest at least one additional AI-controlled mandatory ETB player-target trigger and one AI in-play target activated ability. | The shared AI selector now bypasses the generic target-selection path for pure player targets in ManalinkEh, and the Shandalar targeter also bypasses pure in-play targets after the candidate list is built. |
